 ...
 */
   ...
   */
        ...
        int port() const
        {
            return ntohs((addr.ss_family == AF_INET6) ? reinterpret_cast<const sockaddr_in6*>(&addr)->sin6_port : reinterpret_cast<const sockaddr_in*>(&addr)->sin_port);
        }
 ...
 */
   ...
   */
    ...
    protected:
        explicit Socket(SockType sockfd)
            : sockfd(sockfd)
        {
        }
    };
 ...
 */
   ...
   */
        explicit TCPSocket(SockType sockfd)
            : Socket(sockfd)
        {
        }
        /*!
   ...
   */
        int AtMark() const
        {
            ...
            if (ioctlsocket(sockfd, SIOCATMARK, &atmark) != NO_ERROR)
                return -1;
#else
            int atmark;
