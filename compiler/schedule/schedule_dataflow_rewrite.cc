...
public:
    ...
    ir::CommReducer MutateCommReducer(ir::CommReducer combiner)
    {
        ...
        if (combiner->identity_element.same_as(new_identity) && combiner->identity_element.same_as(new_result)) {
            return combiner;
        } else {
...
{
    return (a->combiner.same_as(b->combiner)) && (a->source.same_as(b->source)) && (a->axis.same_as(b->axis)) && (a->condition.same_as(b->condition));
}
    ...
    const Array<Operation>& readers)
{
    ...
    Tensor cache = compute(
        sugar_tensor->shape, [&sugar_tensor](const Array<Var>& i) {
...
void RebaseNonZeroMinLoop(const Schedule& sch)
{
    ...
    for (Stage s : sch->stages) {
        if (s->attach_type == kInlinedAlready)
            continue;
    ...
    int factor_axis)
{
    ...
    const int factor_axis_pos = factor_axis >= 0 ? factor_axis : static_cast<int>(compute_op->axis.size() + 1) + factor_axis;
    ...
    for (IterVarRelation rel : reduce_stage->relations) {
        ...
        if (const SplitNode* r = rel.as<SplitNode>()) {
            if (touch_map.count(r->parent))
                touched = true;
        } else if (const FuseNode* r = rel.as<FuseNode>()) {
            if (touch_map.count(r->fused))
                touched = true;
