...
namespace schedule {
                ...
                if (r->factor.defined()) {
                    ...
                    Update(p_state, r->outer,
                        Range::make_by_min_extent(
                            0, ceil_div(range_parent->extent, r->factor)),
                        actx);
                } else {
                    Update(p_state, r->outer, Range::make_by_min_extent(0, r->nparts), actx);
                    Update(p_state, r->inner,
                        Range::make_by_min_extent(
                            0, ceil_div(range_parent->extent, r->nparts)),
                        actx);
                }
    ...
    void PassUpDomain(const SplitNode* s,
        ...
        IntSet* parent)
    {
        if (dom_map.count(s->outer) && dom_map.count(s->inner) && dom_map.count(s->parent) && outer.match_range(dom_map.at(s->outer)) && inner.match_range(dom_map.at(s->inner))) {
        ...
        IntSet* inner)
    {
        ...
        if (fused.is_single_point()) {
            ...
            if (!is_zero(outer_min))
                v_outer = v_outer + outer_min;
            if (!is_zero(inner_min))
                v_inner = v_inner + inner_min;
            ...
            *outer = IntSet::interval(outer_min + fused.min() / inner_extent,
            ...
            if (is_zero(Simplify(inner_extent % fused_extent)) && is_zero(Simplify(fused.min() % fused_extent))) {
                ...
                *inner = IntSet::interval(inner_min + fused.min() % inner_extent,
                ...
                if (!is_zero(Simplify(fused_extent % inner_extent)) || !is_zero(Simplify(fused.min() % inner_extent))) {
                    LOG(WARNING) << "fused and original axes are not aligned, this may cause redundant computations";
                }
                *inner = IntSet::range(dom_map.at(s->inner));
            }
            return;
        ...
        bool allow_missing)
    {
        ...
        for (IterVarRelation rel : stage->relations) {
            if (const SplitNode* s = rel.as<SplitNode>()) {
                ...
                int res = 0;
                if (state.count(s->outer))
                    res |= state.at(s->outer);
                if (state.count(s->inner))
                    res |= state.at(s->inner);
                if (state.count(s->fused))
                    res |= state.at(s->fused);
                state[s->fused] = res;
            } else if (const RebaseNode* s = rel.as<RebaseNode>()) {
                if (!state.count(s->parent)) {
                    CHECK(allow_missing);
