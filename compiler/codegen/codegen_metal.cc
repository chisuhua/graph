...
namespace codegen {
    ...
    void CodeGenMetal::AddFunction(LoweredFunc f)
    {
        ...
        for (IterVar iv : f->thread_axis) {
            ...
            var_idmap_[iv->var.get()] = CastFromTo(vname, UInt(thread_index_bits_), iv->var.type());
        }
        // the function scope.
        stream << ") {\n";
    ...
    {
        CHECK(!var_idmap_.count(iv->var.get()));
        var_idmap_[iv->var.get()] = CastFromTo(iv->thread_tag, UInt(thread_index_bits_), iv->var.type());
    }

    void CodeGenMetal::PrintType(Type t, std::ostream& os)
    { // NOLINT(*)
        int lanes = t.lanes();
        if (t.is_handle()) {
            CHECK_EQ(lanes, 1)
    ...
    void CodeGenMetal::VisitExpr_(const Broadcast* op, std::ostream& os)
    { // NOLINT(*)
    ...
    void CodeGenMetal::VisitExpr_(const Call* op, std::ostream& os)
    { // NOLINT(*)
