...
namespace codegen {
    ...
    spirv::Value CodeGenSPIRV::CreateStorageSync(const Call* op)
    {
        ...
        if (sync == "warp") {
            ...
            builder_->MakeInst(
                ...
                builder_->IntImm(type_int, static_cast<int64_t>(spv::ScopeWorkgroup)),
                builder_->IntImm(type_int, static_cast<int64_t>(spv::MemorySemanticsSequentiallyConsistentMask | spv::MemorySemanticsWorkgroupMemoryMask)));
        } else {
    ...
    void CodeGenSPIRV::VisitStmt_(const For* op)
    {
        ...
        uint32_t control = (op->for_type == ForType::Unrolled ? spv::LoopControlUnrollMask : spv::LoopControlMaskNone);
        builder_->MakeInst(
        ...
        spirv::Value one = op->loop_var.type().is_int() ? builder_->IntImm(loop_var.stype, 1) : builder_->UIntImm(loop_var.stype, 1);
    ...
    void CodeGenSPIRV::VisitStmt_(const AttrStmt* op)
    {
        if (op->attr_key == attr::thread_extent) {
            ...
            CHECK(v);
            storage_info_[v].scope = runtime::StorageScope::make(op->value.as<StringImm>()->value);
