...
{
    Array<NodeRef> out_arg_list;
    return BuildStmt(sch, args, binds, simple_mode, &out_arg_list, config);
}

LoweredFunc lower_func(Schedule sch,
    const Array<Tensor>& args,
    const std::string& name,
    const std::unordered_map<Tensor, Buffer>& binds,
    const BuildConfig& config,
...
{
    Array<NodeRef> out_arg_list;
    Stmt stmt = BuildStmt(sch, args, binds, simple_mode, &out_arg_list, config);
