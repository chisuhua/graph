...
namespace codegen {
    ...
    void CodeGenCUDA::BindThreadIndex(const IterVar& iv)
    {
        ...
        var_idmap_[iv->var.get()] = CastFromTo(iv->thread_tag, UInt(32), iv->var.type());
    }

    void CodeGenCUDA::PrintType(Type t, std::ostream& os)
    { // NOLINT(*)
        int lanes = t.lanes();
        if (t.is_handle()) {
            CHECK_EQ(lanes, 1)
    ...
    void CodeGenCUDA::PrintVecElemLoad(
        const std::string& vec, Type t, int i, std::ostream& os)
    { // NOLINT(*)
    ...
    void CodeGenCUDA::VisitExpr_(const Broadcast* op, std::ostream& os)
    { // NOLINT(*)
    ...
    inline void PrintConst(const FloatImm* op, std::ostream& os, CodeGenCUDA* p)
    { // NOLINT(*)
        switch (op->type.bits()) {
        case 64:
        case 32: {
    ...
    void CodeGenCUDA::VisitExpr_(const FloatImm* op, std::ostream& os)
    { // NOLINT(*)
