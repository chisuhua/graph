...
namespace codegen {
    ...
    public:
        void AddFunction(const LoweredFunc& f) final
        {
            ...
            module_->getOrInsertNamedMetadata("nvvm.annotations")
                ->addOperand(llvm::MDNode::get(*ctx_, { llvm::ValueAsMetadata::get(function_), llvm::MDString::get(*ctx_, "kernel"), llvm::ValueAsMetadata::get(ConstInt32(1)) }));
        }
    ...
    runtime::Module BuildNVPTX(Array<LoweredFunc> funcs, std::string target)
    {
        ...
        CHECK(tm->addPassesToEmitFile(
                  pass, dest_ptx, llvm::TargetMachine::CGFT_AssemblyFile)
            == 0)
            << "Cannot emit target CGFT_ObjectFile";
#else
        CHECK(tm->addPassesToEmitFile(
                  pass, dest_ptx, nullptr, llvm::TargetMachine::CGFT_AssemblyFile)
            == 0)
            << "Cannot emit target CGFT_ObjectFile";
#endif
        pass.run(*module);
