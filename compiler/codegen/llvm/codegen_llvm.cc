...
namespace codegen {
    ...
    void CodeGenLLVM::HandleImport(const std::string& code)
    {
        ...
        if (code.length() >= 3 && (code.substr(code.length() - 3) == ".ll" || code.substr(code.length() - 3) == ".bc")) {
        ...
        Type type)
    {
        ...
        if (index.defined()) {
            ...
            if (ramp) {
                ...
                if (arith::GetConstInt(index, &base))
                    width = 1;
            }
        }
