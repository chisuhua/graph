...
void BinaryOpMatchTypes(Expr& lhs, Expr& rhs)
{ // NOLINT(*)
    ...
    } else if ((lhs.type().is_int() && rhs.type().is_int()) || (lhs.type().is_uint() && rhs.type().is_uint())) {
    ...
    } else if ((lhs.type().is_int() && rhs.type().is_uint()) || (lhs.type().is_uint() && rhs.type().is_int())) {
...
Expr operator-(Expr a)
{
    ...
    if (fa)
        return ir::FloatImm::make(a.type(), -fa->value);
...
Expr floor(Expr x)
{
    ...
    const FloatImm* fx = x.as<FloatImm>();
    if (fx)
        return FloatImm::make(x.type(), std::floor(fx->value));
...
Expr ceil(Expr x)
{
    ...
    const FloatImm* fx = x.as<FloatImm>();
    if (fx)
        return FloatImm::make(x.type(), std::ceil(fx->value));
...
Expr round(Expr x)
{
    ...
    const FloatImm* fx = x.as<FloatImm>();
    if (fx)
        return FloatImm::make(x.type(), std::nearbyint(fx->value));
...
Expr trunc(Expr x)
{
    ...
    const FloatImm* fx = x.as<FloatImm>();
    if (fx) {
        return FloatImm::make(x.type(), (fx->value < 0 ? std::ceil(fx->value) : std::floor(fx->value)));
    }
