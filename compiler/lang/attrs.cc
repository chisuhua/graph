...
bool AttrsEqualHandler::VisitAttr_(const ArrayNode* lhs, const NodeRef& other)
{
    if (const auto* rhs = other.as<ArrayNode>()) {
        if (rhs->data.size() != lhs->data.size())
            return false;
        for (size_t i = 0; i < lhs->data.size(); ++i) {
            if (!Equal(NodeRef(lhs->data[i]), NodeRef(rhs->data[i])))
                return false;
        }
    }
    return true;
...
bool AttrsEqualHandler::VisitAttr_(const StrMapNode* lhs, const NodeRef& other)
{
    if (const auto* rhs = other.as<StrMapNode>()) {
        ...
        for (const auto& kv : lhs->data) {
            ...
            if (it == rhs->data.end())
                return false;
            if (!Equal(NodeRef(kv.second), NodeRef(it->second)))
                return false;
        }
    }
    return true;
...
#define TVM_DEFINE_ATTRS_BINOP_EQUAL(NodeName)                                    \
    ...
    {                                                                             \
        if (const auto* rhs = other.as<NodeName>()) {                             \
            if (!Equal(lhs->a, rhs->a))                                           \
                return false;                                                     \
            if (!Equal(lhs->b, rhs->b))                                           \
                return false;                                                     \
            return true;                                                          \
        } else {                                                                  \
            return false;                                                         \
...
bool AttrsEqualHandler::VisitAttr_(const Call* lhs, const NodeRef& other)
{
    if (const auto* rhs = other.as<Call>()) {
        return lhs->name == rhs->name && lhs->type == rhs->type && lhs->call_type == rhs->call_type && Equal(lhs->args, rhs->args);
    } else {
        return false;
    }
}
...
bool AttrsEqualHandler::VisitAttr_(const Select* lhs, const NodeRef& other)
{
    if (const auto* rhs = other.as<Select>()) {
        return Equal(lhs->condition, rhs->condition) && Equal(lhs->true_value, rhs->true_value) && Equal(lhs->false_value, rhs->false_value);
    } else {
        return false;
    }
}
