   ...
   */
        void AddToSelf(SplitExpr other, int64_t scale)
        {
            ...
            for (size_t j = start; j < args.size(); ++j) {
                ...
                if (other->lower_factor == args[j]->lower_factor && other->upper_factor == args[j]->upper_factor && other->DivModeCompatibleTo(args[j]->div_mode)) {
            ...
            for (size_t i = 0; i < args.size(); ++i) {
                ...
                for (size_t j = i + 1; j < args.size(); ++j) {
                    ...
                    if (lhs->upper_factor < rhs->lower_factor)
                        break;
                    if (lhs->upper_factor == rhs->upper_factor && lhs->lower_factor == rhs->lower_factor && lhs->DivModeCompatibleTo(rhs->div_mode)) {
                        // folding same co-efficient.
                        rhs.CopyOnWrite()->scale += lhs->scale;
                        lhs.CopyOnWrite()->scale = 0;
                    } else if (lhs->lower_factor == rhs->upper_factor && rhs->scale != 0 && lhs->scale % rhs->scale == 0 && lhs->lower_factor == (lhs->scale / rhs->scale) * rhs->lower_factor && lhs->DivModeCompatibleTo(rhs->div_mode)) {
   ...
   */
        ...
        SplitDivConst(SplitExpr lhs, int64_t cval, DivMode div_mode)
    {
        ...
        if (cval % lhs->scale == 0) {
            ...
            if (lhs->upper_factor == SplitExprNode::kPosInf || lhs->upper_factor % (lhs->lower_factor * scaled_cval) == 0) {
                ...
                lhs.CopyOnWrite()->index = ModImpl(lhs->index, make_const(lhs.type(), lhs->upper_factor), div_mode);
        ...
        Mutate_(const Div* op, const Expr& self)
    {
        ...
        if (c1.Match(b) && c1.Eval()->value > 0) {
            ...
            if (const auto* psum = a.as<SumExprNode>()) {
                ...
                if (parent_->CanProveGreaterEqual(lhs->Normalize(), 0) && parent_->CanProveGreaterEqual(extra->Normalize(), 0)) {
                    lhs.CopyOnWrite()->DivideBy(cval);
        ...
        SimplifyReduceCombiner(const Reduce* op)
    {
        ...
        mark_used = [&used, &simplified_result, op, &mark_used](size_t idx) {
            ...
            for (size_t i = 0; i < simplified_result.size(); ++i)
                if (!used[i]) {
                    if (ExprUseVar(simplified_result[idx], op->combiner->lhs[i]) || ExprUseVar(simplified_result[idx], op->combiner->rhs[i]))
                        mark_used(i);
                }
        };
        ...
        for (size_t i = 0; i < used.size(); ++i) {
            if (HasSideEffect(op->source[i]) || HasSideEffect(op->combiner->identity_element[i]) || HasSideEffect(op->combiner->result[i])) {
                mark_used(i);
            }
        }
