...
namespace arith {
    ...
    IntervalSet Intersect(Analyzer* analyzer, IntervalSet a, IntervalSet b)
    {
        ...
        if ((max_value.type().is_int() || max_value.type().is_uint()) && (min_value.type().is_int() || min_value.type().is_uint()) && analyzer->CanProveGreaterEqual(min_value - max_value, 1)) {
 ...
 */
        ...
        IntervalSet b)
    {
        if (a->IsSinglePoint() && b->IsSinglePoint()) {
            ...
            if (!res.defined())
                res = Op::make(a->min_value, b->min_value);
    ...
    public:
        ...
        IntervalSet Eval(IntervalSet val)
        {
            ...
            if (static_cast<size_t>(recur_depth_) >= dom_map_.size())
                return val;
            ++recur_depth_;
    ...
    bool IntSet::match_range(const Range& b) const
    {
        ...
        if (!a_int)
            return false;
        return ProveEqual(a_int->min_value, b->min) && ProveEqual(a_int->max_value, b->extent + b->min - 1);
    }
        ...
        const std::unordered_map<const Variable*, IntSet>& dom_map)
    {
        ...
        Expr vmax = s_int->HasUpperBound() ? m.Eval(s_int->max_value).max() : s_int->max_value;
        Expr vmin = s_int->HasLowerBound() ? m.Eval(s_int->min_value).min() : s_int->min_value;
