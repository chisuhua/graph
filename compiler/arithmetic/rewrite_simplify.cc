...
#define TVM_TRY_RECURSIVE_REWRITE_IF(SrcExpr, ResExpr, CondExpr) \
        ...
        Mutate_(const Sub* op, const Expr& self)
    {
        ...
        if (IsIndexType(op->type)) {
            ...
            TVM_TRY_REWRITE_IF((x + c1) / c3 - (x + c2) / c3,
                ...
                CanProveGreaterEqual(x.Eval(), -c2.Eval()->value) && c1.Eval()->value >= c2.Eval()->value && c3.Eval()->value > 0);
        ...
        Mutate_(const Div* op, const Expr& self)
    {
        ...
        if (IsIndexType(op->type)) {
            ...
            TVM_TRY_REWRITE_IF((x / c1 + c2) / c3, (x + c1 * c2) / (c1 * c3),
                c1.Eval()->value > 0 && c2.Eval()->value >= 0 && c3.Eval()->value > 0 && CanProveGreaterEqual(x.Eval(), 0));
            ...
            TVM_TRY_REWRITE_IF((x * c1 + y) / c2, x * (c1 / c2) + y / c2,
                c1.Eval()->value >= 0 && c2.Eval()->value > 0 && c1.Eval()->value % c2.Eval()->value == 0 && CanProveGreaterEqual(x.Eval(), 0) && CanProveGreaterEqual(y.Eval(), 0));
            ...
            TVM_TRY_REWRITE_IF((x * c1 + y + z) / c2, x * (c1 / c2) + (y + z) / c2,
                c1.Eval()->value >= 0 && c2.Eval()->value > 0 && c1.Eval()->value % c2.Eval()->value == 0 && CanProveGreaterEqual(x.Eval(), 0) && CanProveGreaterEqual((y + z).Eval(), 0));

            TVM_TRY_REWRITE_IF((x * c1 - y + z) / c2, x * (c1 / c2) + (z - y) / c2,
                c1.Eval()->value >= 0 && c2.Eval()->value > 0 && c1.Eval()->value % c2.Eval()->value == 0 && CanProveGreaterEqual(x.Eval(), 0) && CanProveGreaterEqual((z - y).Eval(), 0));
            ...
            TVM_TRY_REWRITE_IF((x + c1) / c2, x / c2 + c1 / c2,
                c1.Eval()->value > 0 && c2.Eval()->value > 0 && c1.Eval()->value % c2.Eval()->value == 0 && CanProveGreaterEqual(x.Eval(), 0));
            ...
            TVM_TRY_REWRITE_IF((x * z + y) / z, x + y / z,
                CanProveGreaterEqual(x.Eval(), 0) && CanProveGreaterEqual(y.Eval(), 0) && CanProveGreaterEqual(z.Eval(), 0));
        ...
        Mutate_(const Mod* op, const Expr& self)
    {
        ...
        if (IsIndexType(op->type)) {
            ...
            TVM_TRY_REWRITE_IF((x * c1 + y) % c2, y % c2,
                c2.Eval()->value > 0 && c1.Eval()->value % c2.Eval()->value == 0 && CanProveGreaterEqual((x * c1).Eval(), 0) && CanProveGreaterEqual(y.Eval(), 0));

            TVM_TRY_REWRITE_IF((x + c1) % c2, x % c2,
                c2.Eval()->value > 0 && c1.Eval()->value >= 0 && c1.Eval()->value % c2.Eval()->value == 0 && CanProveGreaterEqual(x.Eval(), 0));

            TVM_TRY_REWRITE_IF((x + y * c1) % c2, x % c2,
                c2.Eval()->value > 0 && c1.Eval()->value % c2.Eval()->value == 0 && CanProveGreaterEqual(x.Eval(), 0) && CanProveGreaterEqual((y * c1).Eval(), 0));
        ...
        if (IsIndexType(op->type)) {
            ...
            if (floordiv(x * c1, c2).Match(ret)) {
                ...
                if (c1val > 0 && c2val > 0) {
                    ...
                    if (c2val % c1val == 0)
                        return (floordiv(x, floordiv(c2, c1))).Eval();
                }
            }
        ...
        Mutate_(const Min* op, const Expr& self)
    {
        ...
        if (IsIndexType(op->type)) {
            ...
            TVM_TRY_REWRITE_IF(min(((x + c1) / c2) * c2, max(x, c2)), max(x, c2),
                c2.Eval()->value > 0 && c1.Eval()->value + 1 == c2.Eval()->value && CanProveGreaterEqual(x.Eval(), 0));
        ...
        Mutate_(const LT* op, const Expr& self)
    {
        ...
        if (IsIndexType(op->a.type())) {
            ...
            // truc div
            TVM_TRY_REWRITE_IF(x * c2 < c1, x<(c1 - 1) / c2 + 1, c1.Eval()->value> 0 && c2.Eval()->value > 0);
            // NOTE: trunc div required
            TVM_TRY_REWRITE_IF(x * c2 < c1, x < c1 / c2,
                c1.Eval()->value <= 0 && c2.Eval()->value > 0);
            // NOTE: trunc div required (euclidean is ok too, floored is not)
            TVM_TRY_REWRITE_IF(x * c2<c1, (c1 - 1) / c2 - 1 < x, c1.Eval()->value> 0 && c2.Eval()->value < 0);
            ...
            // DivMod rules
            // trucdiv
            TVM_TRY_REWRITE_IF(x / c1 < c2, x<c1 * c2, c1.Eval()->value> 0 && c2.Eval()->value > 0);
            // NOTE: trunc div required
            TVM_TRY_REWRITE_IF(x / c1 < c2, x<c1*(c2 - 1) + 1, c1.Eval()->value> 0 && c2.Eval()->value <= 0);
            ...
            TVM_TRY_REWRITE_IF((x / c1) * c1<x, 0 < x % c1, c1.Eval()->value> 0);
            TVM_TRY_REWRITE_IF((x / c1) * c1<x + y, 0 < x % c1 + y, c1.Eval()->value> 0);
            TVM_TRY_REWRITE_IF((x / c1) * c1 < x - y, y<x % c1, c1.Eval()->value> 0);
            ...
            TVM_TRY_REWRITE_IF(floordiv(x, c1) * c1<x + y, 0 < floormod(x, c1) + y, c1.Eval()->value> 0);
            TVM_TRY_REWRITE_IF(floordiv(x, c1) * c1 < x - y, y<floormod(x, c1), c1.Eval()->value> 0);
        ...
        Mutate_(const Select* op, const Expr& self)
    {
        ...
        // normal path
        Expr ret;
        if (cond.same_as(op->condition) && true_value.same_as(op->true_value) && false_value.same_as(op->false_value)) {
            ret = self;
        } else {
        ...
        Mutate_(const Call* op, const Expr& self)
    {
        ...
        if (op->is_intrinsic(ir::intrinsic::tvm_if_then_else)) {
            ...
            if (cond.same_as(op->args[0]) && true_value.same_as(op->args[1]) && false_value.same_as(op->args[2])) {
                ret = self;
            } else {
