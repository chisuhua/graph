...
namespace runtime {
    namespace vm {
        ...
        void InstructionPrint(std::ostream& os, const Instruction& instr)
        {
            ...
            case Opcode::If: {
                os << "if "
                   << "$" << instr.if_op.test << " " << instr.if_op.target << " "
        ...
        TVMContext VirtualMachine::GetParamsContext() const
        {
            ...
            const auto& cit = std::find_if(ctxs.begin(), ctxs.end(), [&fallback_device_type](const TVMContext& c) {
        ...
        {
            DLOG(INFO) << "Executing Function: " << std::endl
                       << func;
