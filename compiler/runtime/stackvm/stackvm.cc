...
#define STACK_VM_STORE(FIELD, DST_TYPE)                                                                 \
    {                                                                                                   \
        int index = code[pc + 1].v_int;                                                                 \
        static_cast<DST_TYPE*>(stack[sp - 1].v_handle)[index] = static_cast<DST_TYPE>(stack[sp] FIELD); \
        sp -= 2;                                                                                        \
        pc += 2;                                                                                        \
    }
...
#define STACK_VM_PRINT_JUMP(CODE)                                          \
    ...
    std::ostream& operator<<(std::ostream& os, const StackVM& vm)
    { // NOLINT(*)
        int64_t pc = 0;
    ...
    bool StackVM::Load(dmlc::Stream* strm)
    {
        ...
        if (!strm->Read(&str_data))
            return false;
        if (!strm->Read(&extern_func_name))
            return false;
        if (!strm->Read(&heap_id_name))
            return false;
        if (!strm->Read(&heap_size))
            return false;
        if (!strm->Read(&stack_size))
            return false;
        this->InitCache();
        return true;
    }
    ...
    void StackVM::Run(State* s) const
    {
        ...
        while (pc < code_size) {
            ...
            case CALL_PACKED_LOWERED: {
                ...
                GetExtern(s, call_fid).CallPacked(runtime::TVMArgs(value_stack + begin, type_stack + begin, num_args), &rv);
                sp = sp - 1;
                stack[sp] = rv.value();
                pc += 4;
                break;
