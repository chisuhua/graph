...
namespace runtime {
    ...
    public:
        ...
        {
            std::string token_file = GetCacheDir() + "/" + GetFileBasename(enclave_file) + ".token";
    ...
    namespace sgx {
        ...
        TVM_REGISTER_GLOBAL("__sgx_println__")
            .set_body([](TVMArgs args, TVMRetValue* rv) {
                ...
                for (int i = 0; i < args.num_args; ++i) {
                    ...
                    case kDLInt:
                        msg << static_cast<int64_t>(args[i]);
                        break;
                    case kDLUInt:
                        msg << static_cast<uint64_t>(args[i]);
                        break;
                    case kDLFloat:
                        msg << static_cast<double>(args[i]);
                        break;
                    case kStr:
                    case kBytes: {
        ...
        TVM_REGISTER_GLOBAL("__sgx_reserve_space__")
            .set_body([](TVMArgs args, TVMRetValue* rv) {
                ...
                if (buf_size >= num_bytes && buf_align >= alignment)
                    *rv = nullptr;
