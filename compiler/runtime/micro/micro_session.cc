...
namespace runtime {
        ...
        int port)
    {
        ...
        void* workspace_start_addr = low_level_device_->ToDevPtr(workspace_start_offset).cast_to<void*>();
        void* workspace_end_addr = low_level_device_->ToDevPtr(workspace_end_offset).cast_to<void*>();
    ...
    {
        int32_t (*func_dev_addr)(void*, void*, int32_t) = reinterpret_cast<int32_t (*)(void*, void*, int32_t)>(
        ...
        DevPtr args_addr = low_level_device()->ToDevPtr(GetAllocator(SectionKind::kArgs)->curr_end_offset());
        ...
        DevBaseOffset stream_dev_offset = GetAllocator(SectionKind::kArgs)->Allocate(encoder.buf_size());
    ...
    BinaryInfo MicroSession::LoadBinary(const std::string& binary_path, bool patch_dylib_pointers)
    {
        ...
        CHECK(text_section.start != nullptr && rodata_section.start != nullptr && data_section.start != nullptr && bss_section.start != nullptr)
