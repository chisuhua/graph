...
namespace runtime {
    ...
    public:
            ...
            void* nd_handle)
        {
            ...
            data->dl_tensor.ctx.device_type = static_cast<DLDeviceType>(
                static_cast<int>(tensor->ctx.device_type) + kRPCSessMask * (sess->table_index() + 1));
            // check strides.
        ...
        TVMRetValue* rv)
    {
        ...
        if (tcode == kFuncHandle) {
            ...
            *rv = PackedFunc([wf](TVMArgs args, TVMRetValue* rv) {
            ...
            std::shared_ptr<RPCModuleNode> n = std::make_shared<RPCModuleNode>(handle, sess);
            *rv = Module(n);
    ...
    {
        std::shared_ptr<RPCModuleNode> n = std::make_shared<RPCModuleNode>(nullptr, sess);
        return Module(n);
    }
    ...
    TVM_REGISTER_GLOBAL("rpc._LoadRemoteModule")
        .set_body([](TVMArgs args, TVMRetValue* rv) {
            ...
            std::shared_ptr<RPCModuleNode> n = std::make_shared<RPCModuleNode>(mhandle, sess);
            *rv = Module(n);
        });
    ...
    TVM_REGISTER_GLOBAL("rpc._ImportRemoteModule")
        .set_body([](TVMArgs args, TVMRetValue* rv) {
            ...
            Module child = args[1];
            CHECK(!std::strcmp(parent->type_key(), "rpc") && !std::strcmp(child->type_key(), "rpc"));
