    ...
    namespace vulkan {
            ...
            TVMStreamHandle stream)
        {
            ...
            if (from_dev_type == kDLVulkan && to_dev_type == kDLVulkan) {
                ...
                barrier_info.dstAccessMask = (VK_ACCESS_TRANSFER_READ_BIT | VK_ACCESS_TRANSFER_WRITE_BIT | VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT);
        ...
        VulkanCommandPool* VulkanThreadEntry::CommandPool(int device_id)
        {
            ...
            if (pool_[device_id] == nullptr) {
                const VulkanContext& vctx = VulkanWorkspace::Global()->context_[device_id];
        ...
        void FindMemoryTypeIndex(VulkanContext* vctx)
        {
            ...
            for (uint32_t k = 0; k < prop.memoryTypeCount; ++k) {
                ...
                // host visible
                if (!(ty.propertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT))
                    continue;
                // match copy requirment
                if (!(req_staging.memoryTypeBits & (1 << k)))
                    continue;
                if (heap_size < 1024)
                    continue;
                int rank = 0;
                ...
                if (rank > win_rank) {
                    ...
                    vctx->coherent_staging = ty.propertyFlags & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
                }
            }
            ...
            for (uint32_t k = 0; k < prop.memoryTypeCount; ++k) {
                ...
                size_t heap_size = prop.memoryHeaps[ty.heapIndex].size;
                // host visible
                if (!(ty.propertyFlags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT))
                    continue;
                // match copy requirment
                if (!(req_staging.memoryTypeBits & (1 << k)))
                    continue;
