...
namespace contrib {
    ...
    TVM_REGISTER_GLOBAL("tvm.contrib.nnpack.convolution_inference")
        .set_body([](TVMArgs args, TVMRetValue* ret) {
            ...
            void* workspace_buffer = cpu_api->AllocWorkspace(ctx, workspace_elements * sizeof(float), type_hint);
            ...
            for (auto n = 0; n < input->shape[0]; ++n) {
                nnp_status status = nnp_convolution_inference(
                    ...
                    stride_size,
                    static_cast<float*>(input->data) + n * input->shape[1] * input->shape[2] * input->shape[3],
                    static_cast<float*>(kernel->data),
                    bias ? static_cast<float*>(bias->data) : zero_bias->data(),
                    static_cast<float*>(output->data) + n * output->shape[1] * output->shape[2] * output->shape[3],
