...
namespace ir {
    ...
    inline Array<IterVar> MutateIterVarArr(Array<IterVar> rdom, IRMutator* m)
    {
        ...
        for (size_t i = 0; i < rdom.size(); i++) {
            ...
            if (!r->min.same_as(new_min))
                changed = true;
            if (!r->extent.same_as(new_extent))
                changed = true;
...
#define DISPATCH_TO_MUTATE_STMT(OP)                                  \
    ...
    Stmt IRMutator::Mutate_(const Allocate* op, const Stmt& s)
    {
        ...
        if (all_extents_unmodified && body.same_as(op->body) && condition.same_as(op->condition) && new_expr.same_as(op->new_expr)) {
            return s;
        } else {
    ...
    Stmt IRMutator::Mutate_(const Realize* op, const Stmt& s)
    {
        ...
        Stmt body = m->Mutate(op->body);
        Expr condition = m->Mutate(op->condition);
        if (!bounds_changed && body.same_as(op->body) && condition.same_as(op->condition)) {
            return s;
        } else {
...
#define DEFINE_BIOP_EXPR_MUTATE_(OP)                     \
    ...
    Expr IRMutator::Mutate_(const Reduce* op, const Expr& e)
    {
        ...
        if (op->axis.same_as(new_axis) && op->source.same_as(new_source) && op->condition.same_as(new_cond)) {
            return e;
        } else {
            return Reduce::make(
