    ...
    public:
        ...
        Expr Mutate_(const Call* op, const Expr& e) final
        {
            if (op->is_intrinsic(intrinsic::tvm_access_ptr)) {
                ...
                Expr stride = it->second / make_const(offset.type(), dtype.lanes());
        ...
        Stmt Mutate_(const AttrStmt* op, const Stmt& s) final
        {
            ...
            } else if (!allow_share_ && !vt_loop_injected_ && (op->attr_key == attr::coproc_uop_scope || op->attr_key == attr::coproc_scope)) {
                return InjectVTLoop(s, true);
            } else {
        ...
        Stmt Mutate_(const Allocate* op, const Stmt& s) final
        {
            ...
            for (size_t i = 0; i < op->extents.size(); i++) {
                ...
                if (!new_ext.same_as(op->extents[i]))
                    changed = true;
