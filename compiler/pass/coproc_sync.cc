...
namespace ir {
            ...
            bool force_sync_at_end)
        {
            ...
            auto find_conflict = [&](const AccessEntry& acc) {
                for (const AccessEntry& x : co_access) {
                    if (x.buffer.same_as(acc.buffer) && ((acc.type == kRead && x.type == kWrite) || acc.type == kWrite)) {
                        return true;
                    }
                }
                return false;
            ...
            if (loop != nullptr && !sync_at_end) {
                ...
                for (size_t i = 0; i < seq.size(); ++i) {
                    ...
                    if (sync_.count(s.stmt) || sync_at_end)
                        break;
                }
            }
    ...
    private:
            ...
            std::vector<std::pair<int, int>>* next_enter_pop)
        {
            ...
            // quick path
            if (prev.exit_push.size() == 0 && next.enter_pop.size() == 0 && prev.exit_ctx.size() == 1 && next.enter_ctx.size() == 1) {
