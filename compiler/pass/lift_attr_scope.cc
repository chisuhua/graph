...
namespace ir {
    ...
    public:
        ...
        Stmt Mutate_(const Block* op, const Stmt& s) final
        {
            ...
            seq = MutateSeq(seq);
            if (seq.size() == 2 && seq[0].same_as(op->first) && seq[1].same_as(op->rest)) {
                return s;
            }
            return MergeSeq(seq);
        }
        ...
        Stmt Mutate_(const IfThenElse* op, const Stmt& s) final
        {
            ...
            if (attr_node_.defined() && attr_value_.defined() && first_node.defined() && first_value.defined() && attr_node_.same_as(first_node) && ValueSame(attr_value_, first_value)) {
    ...
    private:
        ...
        std::vector<Stmt> MutateSeq(const std::vector<Stmt>& seq)
        {
            ...
            for (const Stmt& stmt : seq) {
                ...
                if (attr_node_.defined() && attr_value_.defined() && curr_node.defined() && curr_value.defined() && attr_node_.same_as(curr_node) && ValueSame(attr_value_, curr_value)) {
