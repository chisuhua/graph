...
namespace ir {
    ...
    public:
        ...
        void Visit_(const AttrStmt* op) final
        {
            ...
            if (op->attr_key == attr::thread_extent && !in_thread_env_) {
                ...
                alloc_info_[buf].storage_scope = StorageScope::make(op->value.as<StringImm>()->value);
                IRVisitor::Visit_(op);
            } else {
                IRVisitor::Visit_(op);
            }
    ...
    public:
        ...
        void Visit_(const Load* op) final
        {
            ...
            if (src_ == buf) {
                if (store_ == nullptr || store_->value.type() != op->type || !ir::Equal(store_->index, op->index)) {
                    result_ = false;
                    return;
                }
            }
    ...
    public:
        ...
        Stmt Mutate_(const AttrStmt* op, const Stmt& s) final
        {
            ...
            } else if (op->attr_key == attr::thread_extent || op->attr_key == attr::virtual_thread || attr::IsPragmaKey(op->attr_key)) {
    ...
    private:
            ...
            const std::unordered_map<const Variable*, AllocEntry>& alloc_info)
        {
            ...
            for (size_t i = 0; i < seq.size(); ++i) {
                        ...
                        if (detect_inplace) {
                            ...
                            for (const Variable* src : it->second.kill) {
                                if (!inplace_flag.count(src) && alloc_map_.count(src)) {
                                    ...
                                    if (src_entry->scope == ae.storage_scope && src_entry->attach_scope_ == thread_scope_ && src_entry->elem_type == ae.alloc->type.element_of() && visitor.Check(s.stmt, var, src)) {
                                        uint64_t const_nbits = static_cast<uint64_t>(ae.alloc->constant_allocation_size()) * ae.alloc->type.bits() * ae.alloc->type.lanes();
                ...
                if (s.stmt->is_type<AttrStmt>()) {
                    ...
                    if (op->attr_key == attr::thread_extent || op->attr_key == attr::virtual_thread || attr::IsPragmaKey(op->attr_key)) {
                        PlanNewScope(op);
                    } else {
            ...
            const StorageScope& scope)
        {
            ...
            if (const_nbits != 0) {
                ...
                for (auto it = mid; it != begin;) {
                    ...
                    if (e->scope != scope)
                        continue;
                    if (e->elem_type != op->type.element_of())
                        continue;
                    e->const_nbits = std::max(const_nbits, e->const_nbits);
                    const_free_map_.erase(it);
                    return e;
        ...
        void Free(const Variable* var)
        {
            ...
            if (e->scope.tag.length() == 0) {
                ...
                if (e->scope.rank >= StorageRank::kWarp || e->allocs[0]->type.is_handle())
                    return;
    ...
    public:
        ...
        Stmt Mutate_(const Allocate* op, const Stmt& s) final
        {
            ...
            if (tvec.size() == 1 && tvec[0].element_of() == op->type.element_of() && tvec[0].lanes() % op->type.lanes() == 0 && tvec[0].lanes() != op->type.lanes()) {
