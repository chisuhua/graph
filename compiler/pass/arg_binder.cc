...
namespace ir {
        ...
        const std::string& arg_name)
    {
        ...
        Expr cond = (TVMArrayGet(UInt(8), handle, intrinsic::kArrTypeCode) == UIntImm::make(UInt(8), dtype.code()) && TVMArrayGet(UInt(8), handle, intrinsic::kArrTypeBits) == UIntImm::make(UInt(8), dtype.bits()) && TVMArrayGet(UInt(16), handle, intrinsic::kArrTypeLanes) == UIntImm::make(UInt(16), dtype.lanes()));
        ...
        if (buffer->strides.size() == 0) {
            ...
            if (conds.size() != 0) {
                Stmt check = AssertStmt::make(arith::ComputeReduce<ir::And>(conds, Expr()),
        ...
        if (arith::GetConst(buffer->elem_offset, &const_offset)) {
            ...
            if (Bind_(buffer->elem_offset,
                    cast(buffer->elem_offset.type(),
                        (TVMArrayGet(UInt(64), handle, intrinsic::kArrByteOffset) / make_const(UInt(64), data_bytes))),
