        ...
        Stmt Mutate_(const Store* op, const Stmt& s) final
        {
            ...
            if (store_scope_bound_collector_.size()) {
                ...
                if (!condition.as<StringImm>()) {
                    ...
                    Stmt then_case = Store::make(op->buffer_var, op->value, op->index, op->predicate);
                    Stmt else_case = AssertStmt::make(condition, StringImm::make(error_message_), nop);
    ...
    private:
            ...
            const Type& type)
        {
            ...
            for (size_t i = 0; i < new_shape.size(); ++i) {
                if (!new_shape[0].defined() || !new_shape[i].type().is_scalar() || is_negative_const(new_shape[i])) {
                    return;
                }
            }
            ...
            for (size_t i = 1; i < new_shape.size(); ++i) {
                ...
                shape = Mul::make(shape, Mul::make(make_const(UInt(64), type.lanes()), Cast::make(UInt(64), new_shape[i])));
            }
        ...
        bool IndexIsValid(const Expr& index) const
        {
            ...
            if (const Ramp* ramp_index = index.as<Ramp>()) {
                return ramp_index->base.defined() && ramp_index->base.type().is_scalar() && ramp_index->stride.defined() && ramp_index->stride.type().is_scalar() && (ramp_index->lanes > 0);
            }
            return true;
        }
        ...
        {
            return buffer_var.defined() && mem_to_shape_.count(buffer_var.get()) && IndexIsValid(index) && !unsafe_rewritten_;
        }
        ...
        Expr MakeCondition()
        {
            ...
            for (size_t i = 0; i < store_scope_bound_collector_.size(); ++i) {
                ...
                if (const Ramp* ramp_index = index.as<Ramp>()) {
                    ...
                    index = Add::make(
                        ramp_index->base,
                        Mul::make(ramp_index->stride, make_const(ramp_index->stride.type(), ramp_index->lanes - 1)));
                }
                ...
                Expr current_condition = And::make(GE::make(index, lower_bound), LT::make(index, upper_bound));
                condition = !i ? current_condition : And::make(condition, current_condition);
            }
            return condition;
        }
