 ...
 */
        ...
        public:
            ...
            void Run()
            {
                if (!IsGPUDevice(dev_type_) && !IsFPGADevice(dev_type_))
                    return;
        ...
        protected:
            ...
            {
                if (!InThreadEnv() && (op->attr_key == attr::thread_extent || op->attr_key == attr::pipeline_exec_scope)) {
                    EnterThreadEnv();
                    IRVisitor::Visit_(op);
                    ExitThreadEnv();
                } else {
            ...
            bool IsFromFunctionArgs(const Variable* var) const
            {
                ...
                while (true) {
                    ...
                    if (!C || C->name != intrinsic::tvm_struct_get)
                        return false;
            ...
            {
                return kDLGPU == dev_type || kDLOpenCL == dev_type || kDLVulkan == dev_type || kDLMetal == dev_type || kDLROCM == dev_type || kOpenGL == dev_type;
            }
