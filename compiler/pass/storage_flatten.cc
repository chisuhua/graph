...
namespace ir {
    ...
    public:
        ...
        Stmt Mutate_(const AttrStmt* op, const Stmt& s) final
        {
            ...
            } else if (op->attr_key == attr::double_buffer_scope && op->node.node_->derived_from<OperationNode>()) {
    ...
    private:
        ...
        bool ShapeIsValid(const Array<Expr>& shape)
        {
            ...
            for (size_t i = 0; i < shape.size(); ++i) {
                if (!shape[i].defined() || !shape[i].type().is_scalar() || is_negative_const(shape[i])) {
                    return false;
                }
            }
            return true;
