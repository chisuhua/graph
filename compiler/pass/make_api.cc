        ...
        bool is_restricted)
    {
        ...
        for (int i = 0; i < static_cast<int>(api_args.size()); ++i) {
            ...
            if (i < num_packed_args) {
                ...
                seq_init.emplace_back(LetStmt::make(
                    tcode, Load::make(Int(32), v_packed_arg_type_ids, IntImm::make(Int(32), i), const_true(1)),
                    nop));
                Type t = v_arg.type();
                if (t.is_handle()) {
                    ...
                    seq_check.emplace_back(
                        AssertStmt::make(tcode == kHandle || tcode == kNDArrayContainer || tcode == kArrayHandle || tcode == kNull, msg.str(), nop));
        ...
        if (vmap.count(device_id.get())) {
            ...
            Stmt set_device = IfThenElse::make(
                device_type != kDLCPU, Evaluate::make(Call::make(Int(32), intrinsic::tvm_call_packed, { StringImm::make(runtime::symbol::tvm_set_device), device_type, device_id }, Call::Intrinsic)));
