...
namespace ir {
    ...
    private:
        ...
        {
            Array<Expr> m = arith::DetectLinearEquation(index, { warp_index_ });
            CHECK_EQ(m.size(), 2U)
        ...
        void Visit_(const AttrStmt* op) final
        {
            if (op->attr_key == attr::thread_extent) {
                ...
                if (iv->thread_tag == "threadIdx.x") {
                    int value;
                    CHECK(arith::GetConstInt(op->value, &value) && value == warp_size_)
    ...
    protected:
        ...
        std::pair<Expr, Expr> SplitIndexByGroup(const Expr& index)
        {
            if (index.type().lanes() != 1) {
                ...
                local_index = Ramp::make(local_index, make_const(local_index.type(), 1), index.type().lanes());
