...
#define DEFINE_BIOP_EXPR_CMP_(OP)                          \
    ...
    {                                                      \
        ...
        if (CompareExpr(op->a, rhs->a) != 0)               \
            return;                                        \
        if (CompareExpr(op->b, rhs->b) != 0)               \
            return;                                        \
    }
    ...
    public:
        ...
        void VisitExpr(const Expr& n, const Expr& other) override
        {
            ...
            if (n.same_as(other))
                return;
            if (CompareValue(n->type_index(), other->type_index()) != 0)
                return;
            if (CompareType(n.type(), other.type()) != 0)
                return;
        ...
        void VisitStmt_(const LetStmt* op, const Stmt& other) final
        {
            ...
            if (CompareExpr(op->value, rhs->value) != 0)
                return;
            if (tie_def_) {
                vmap_[op->var.get()] = rhs->var.get();
            } else {
                if (CompareExpr(op->var, rhs->var) != 0)
                    return;
            }
            if (CompareStmt(op->body, rhs->body) != 0)
                return;
        }
        ...
        void VisitStmt_(const AttrStmt* op, const Stmt& other) final
        {
            ...
            if (CompareString(op->attr_key, rhs->attr_key) != 0)
                return;
            if (CompareNodeRef(op->node, rhs->node) != 0)
                return;
            if (CompareExpr(op->value, rhs->value) != 0)
                return;
            if (CompareStmt(op->body, rhs->body) != 0)
        ...
        void VisitStmt_(const IfThenElse* op, const Stmt& other) final
        {
            ...
            if (CompareExpr(op->condition, rhs->condition) != 0)
                return;
            if (CompareStmt(op->then_case, rhs->then_case) != 0)
                return;
            if (CompareStmt(op->else_case, rhs->else_case) != 0)
                return;
        }
        ...
        void VisitStmt_(const For* op, const Stmt& other) final
        {
            ...
            if (CompareExpr(op->min, rhs->min) != 0)
                return;
            if (CompareExpr(op->extent, rhs->extent) != 0)
                return;
            if (tie_def_) {
                vmap_[op->loop_var.get()] = rhs->loop_var.get();
            } else {
                if (CompareExpr(op->loop_var, rhs->loop_var) != 0)
                    return;
            }
            if (CompareStmt(op->body, rhs->body) != 0)
                return;
        ...
        void VisitStmt_(const Allocate* op, const Stmt& other) final
        {
            ...
            if (tie_def_) {
                ...
                if (CompareExpr(op->buffer_var, rhs->buffer_var) != 0)
                    return;
            }
            if (CompareType(op->type, rhs->type) != 0)
                return;
            if (CompareArray(op->extents, rhs->extents) != 0)
                return;
            if (CompareExpr(op->condition, rhs->condition) != 0)
                return;
            if (CompareStmt(op->body, rhs->body) != 0)
                return;
            if (CompareExpr(op->new_expr, rhs->new_expr) != 0)
                return;
            if (CompareString(op->free_function, rhs->free_function) != 0)
                return;
        }
        ...
        void VisitStmt_(const Store* op, const Stmt& other) final
        {
            ...
            if (CompareExpr(op->value, rhs->value) != 0)
                return;
            if (CompareExpr(op->index, rhs->index) != 0)
                return;
            if (CompareExpr(op->predicate, rhs->predicate) != 0)
                return;
        }
        ...
        void VisitStmt_(const AssertStmt* op, const Stmt& other) final
        {
            ...
            if (CompareExpr(op->condition, rhs->condition) != 0)
                return;
            if (CompareExpr(op->message, rhs->message) != 0)
                return;
            if (CompareStmt(op->body, rhs->body) != 0)
                return;
        }
        ...
        void VisitStmt_(const ProducerConsumer* op, const Stmt& other) final
        {
            ...
            if (CompareNodeRef(op->func, rhs->func) != 0)
                return;
            if (CompareValue(op->is_producer, rhs->is_producer) != 0)
                return;
            if (CompareStmt(op->body, rhs->body) != 0)
                return;
        }
        ...
        void VisitStmt_(const Provide* op, const Stmt& other) final
        {
            ...
            if (CompareNodeRef(op->func, rhs->func) != 0)
                return;
            if (CompareValue(op->value_index, rhs->value_index) != 0)
                return;
            if (CompareExpr(op->value, rhs->value) != 0)
                return;
            if (CompareArray(op->args, rhs->args) != 0)
                return;
        }
        ...
        void VisitStmt_(const Realize* op, const Stmt& other) final
        {
            ...
            if (CompareType(op->type, rhs->type) != 0)
                return;
            if (CompareRegion(op->bounds, rhs->bounds) != 0)
                return;
            if (CompareStmt(op->body, rhs->body) != 0)
                return;
        }
        ...
        void VisitStmt_(const Block* op, const Stmt& other) final
        {
            ...
            if (CompareStmt(op->first, rhs->first) != 0)
                return;
            if (CompareStmt(op->rest, rhs->rest) != 0)
                return;
        }
        ...
        void VisitExpr_(const Let* op, const Expr& other) final
        {
            ...
            if (CompareExpr(op->value, rhs->value) != 0)
                return;
            if (CompareExpr(op->body, rhs->body) != 0)
                return;
        }
        ...
        void VisitExpr_(const Call* op, const Expr& other) final
        {
            ...
            if (CompareString(op->name, rhs->name))
                return;
            if (CompareArray(op->args, rhs->args))
                return;
            if (CompareValue(op->call_type, rhs->call_type) != 0)
                return;
            if (CompareNodeRef(op->func, rhs->func) != 0)
                return;
            if (CompareValue(op->value_index, rhs->value_index) != 0)
        ...
        void VisitExpr_(const Reduce* op, const Expr& other) final
        {
            ...
            if (CompareCommReducer(op->combiner, rhs->combiner) != 0)
                return;
            if (CompareValue(op->axis.size(), rhs->axis.size()) != 0)
                return;
            if (CompareValue(op->value_index, rhs->value_index) != 0)
                return;
            for (size_t i = 0; i < op->axis.size(); ++i) {
                ...
                if (tie_def_) {
                    ...
                    if (CompareExpr(op->axis[i]->var, rhs->axis[i]->var) != 0)
                        return;
                }
            }
            if (CompareExpr(op->condition, rhs->condition) != 0)
                return;
            if (CompareArray(op->source, rhs->source) != 0)
                return;
        }
        ...
        void VisitExpr_(const Select* op, const Expr& other) final
        {
            ...
            if (CompareExpr(op->condition, rhs->condition) != 0)
                return;
            if (CompareExpr(op->true_value, rhs->true_value) != 0)
                return;
            if (CompareExpr(op->false_value, rhs->false_value) != 0)
                return;
        }
        ...
        void VisitExpr_(const Ramp* op, const Expr& other) final
        {
            ...
            if (CompareExpr(op->base, rhs->base) != 0)
                return;
            if (CompareExpr(op->stride, rhs->stride) != 0)
                return;
            if (CompareValue(op->lanes, rhs->lanes) != 0)
                return;
        }
        ...
        void VisitExpr_(const Shuffle* op, const Expr& other) final
        {
            ...
            if (CompareArray(op->vectors, rhs->vectors) != 0)
                return;
            if (CompareArray(op->indices, rhs->indices) != 0)
                return;
        }
    ...
    private:
        ...
        int CompareArray(const Array<Expr>& lhs, const Array<Expr>& rhs)
        {
            if (order_ != 0)
                return order_;
            if (CompareValue(lhs.size(), rhs.size()) != 0)
                return order_;
            for (size_t i = 0; i < lhs.size(); ++i) {
                if (CompareExpr(lhs[i], rhs[i]) != 0)
                    return order_;
            }
            return order_;
        }
        ...
        int CompareRegion(const Region& lhs, const Region& rhs)
        {
            ...
            if (CompareValue(lhs.size(), rhs.size()) != 0)
                return order_;
            for (size_t i = 0; i < lhs.size(); ++i) {
                if (CompareExpr(lhs[i]->min, rhs[i]->min) != 0)
                    return order_;
                if (CompareExpr(lhs[i]->extent, rhs[i]->extent) != 0)
                    return order_;
            }
            return order_;
        }
        ...
        int CompareType(const Type& lhs, const Type& rhs)
        {
            ...
            if (lhs == rhs)
                return order_;
            if (CompareValue(lhs.code(), rhs.code()) != 0)
                return order_;
            if (CompareValue(lhs.bits(), rhs.bits()) != 0)
                return order_;
            if (CompareValue(lhs.lanes(), rhs.lanes()) != 0)
                return order_;
            return order_;
        }
        ...
        int CompareCommReducer(const CommReducer& lhs, const CommReducer& rhs)
        {
            ...
            if (lhs == rhs)
                return order_;
            if (CompareValue(lhs->lhs.size(), rhs->lhs.size()) != 0)
                return order_;
            if (CompareValue(lhs->rhs.size(), rhs->rhs.size()) != 0)
                return order_;
            IRDeepCompare cmp;
            if (tie_def_) {
                ...
                for (size_t i = 0; i < lhs->lhs.size(); ++i) {
                    if (CompareExpr(lhs->lhs[i], rhs->lhs[i]) != 0)
                        return order_;
                }
                for (size_t i = 0; i < lhs->lhs.size(); ++i) {
                    if (CompareExpr(lhs->rhs[i], rhs->rhs[i]) != 0)
                        return order_;
                }
            }
