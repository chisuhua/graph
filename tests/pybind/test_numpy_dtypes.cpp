...
void test_format_descriptors(py::module &m)
{
    py::dtype ld = py::dtype::of<long double>();
    std::string ld_id = py::type_id<long double>();
    py::print(ld_id);
    py::print(ld.attr("alignment"));

    std::string ldbl_fmt;
    if (ld.attr("alignment").cast<int>() > 4) {
        ldbl_fmt = "4x" + ld.attr("char").cast<std::string>();
    } else {
        ldbl_fmt = ld.attr("char").cast<std::string>();
    }
    py::print(ldbl_fmt);

    std::string ss_fmt = "^T{?:bool_:3xI:uint_:f:float_:" + ldbl_fmt + ":ldbl_:}";

    py::dtype dbl = py::dtype::of<double>();

    std::string partial_fmt = ("^T{?:bool_:3xI:uint_:f:float_:" +
                   std::to_string(4 * (dbl.attr("alignment").cast<int>() > 4) + dbl.itemsize() + 8 * (ld.attr("alignment").cast<int>() > 8)) +
                   "xg:ldbl_:}");
    py::print(partial_fmt);

    std::string nested_extra = std::to_string(std::max(8, ld.attr("alignment").cast<int>()));

    py::list lst = m.attr("print_format_descriptors")();
    CHECK_EQ(lst[0].cast<std::string>(), ss_fmt);
}

py::dtype simple_dtype()
{
    auto ld = py::dtype("longdouble");

    py::list names, offsets, formats;
    names.append(py::str("bool_")); names.append(py::str("uint_"));
    names.append(py::str("float_")); names.append(py::str("ldbl_"));

    formats.append(py::dtype("?")); formats.append(py::dtype("u4"));
    formats.append(py::dtype("f4")); formats.append(py::dtype("u" + std::to_string(ld.itemsize())));

    offsets.append(py::int_(0)); offsets.append(py::int_(4));
    offsets.append(py::int_(8)); offsets.append(py::int_(16));

    py::print(py::str(py::dtype(names, formats, offsets, 20)));

    // return py::dtype::of<SimpleStruct>();
}

void test_dtype(py::module &m, py::module &np)
{
    py::print(py::str(py::dtype("int32")));
    py::print(py::str(py::dtype::of<int>()));

    // py::dtype::of<SimpleStruct>()
    CHECK_EQ(py::str(py::dtype("int32")).cast<std::string>(),  py::str(np.attr("dtype")("int32")).cast<std::string>());
    CHECK_EQ(py::str(py::dtype("float64")).cast<std::string>(),  py::str(np.attr("dtype")("float64")).cast<std::string>());
    CHECK_EQ(py::str(py::dtype("bool")).cast<std::string>(),  py::str(np.attr("dtype")("bool")).cast<std::string>());
    CHECK_EQ(py::str(py::dtype::from_args(py::str("bool"))).cast<std::string>(),  py::str(np.attr("dtype")("bool")).cast<std::string>());

    py::list names, offsets, formats;
    py::dict dict;
    names.append(py::str("a")); names.append(py::str("b"));
    offsets.append(py::int_(1)); offsets.append(py::int_(10));
    formats.append(py::dtype("int32")); formats.append(py::dtype("float64"));
    py::print(py::str(py::dtype(names, formats, offsets, 20)));

    dict["names"] = names;
    dict["offsets"] = offsets;
    dict["formats"] = formats;
    py::print(py::str(py::dtype::from_args(dict)));

    py::print(py::str(py::dtype(py::buffer_info((void *) 0, sizeof(unsigned int), "I", 1))));

    py::print(py::str(py::dtype(py::buffer_info((void *) 0, 0, "T{i:a:f:b:}", 1))));

}


TEST(numpy_dtypes, simple_dtype)
{
    py::scoped_interpreter guard{};
    py::module m = py::module::import("numpy_dtypes");
    py::module np = py::module::import("numpy");
    auto ld = np.attr("dtype")(np.attr("longdouble"));
    py::print(np.attr("longdouble"));
    py::print(ld);
    auto ld_c = py::type_id<long double>();
    py::print(py::str(ld_c));
    CHECK_EQ(py::str(ld_c).cast<std::string>(), "long double");

    test_format_descriptors(m);
    test_dtype(m, np);
}

/*
TEST(numpy_dtypes, test_format_descriptors)
{
    py::scoped_interpreter guard{};
    py::module m = py::module::import("numpy_dtypes");
    ...
    // py::module np = py::module::import("numpy");
    // auto ld = np.attr("dtype")(np.attr("longdouble"));
    // auto ldbl_fmt = ld.attr("char");
    // if (ld.attr("alignment").cast<int>() > 4)
    //     py::print("alignment > 4 ");
    // py::print(py::str(ld));
}
*/

PYBIND11_EMBEDDED_MODULE(numpy_dtypes, m) {
    try { py::module::import("numpy"); }
    catch (...) {
        std::cout << "faild to import numpy\n";
        return;
    }
