...
PYBIND11_EMBEDDED_MODULE(stl, m) {
    // test_vector
...
TEST(stl, test_vector) {
    py::scoped_interpreter guard{};
    auto m = py::module::import("stl");
    py::list lst = m.attr("cast_vector")();
    CHECK_EQ(py::str(lst).cast<std::string>(), "[1]");
    lst.append(2);
    CHECK_EQ(m.attr("load_vector")(lst).cast<bool>(), true);
    // CHECK_EQ(m.attr("load_vector")(py::make_tuple(*lst)).cast<bool>(), true);
}
TEST(stl, test_move_out_container) {
    py::scoped_interpreter guard{};
    auto m = py::module::import("stl");
    py::object c = m.attr("MoveOutContainer")();
    py::list lst = c.attr("move_list");
    std::list<py::object> values;
    std::transform(lst.begin(), lst.end(), values.begin(), [](py::handle o) {
            // std::cout << o.attr("value").cast<int>()
            //          << ":\n";
            return o.attr("value");
            });
    // FIXME
    ...
    // CHECK_EQ(py::str(values).cast<std::string>(), R"([0, 1, 2])");
}

#ifdef PYBIND11_HAS_OPTIONAL
TEST(stl, test_optional) {
    py::scoped_interpreter guard{};
    auto m = py::module::import("stl");
    CHECK_EQ(m.attr("double_or_zero")(nullptr).cast<int>(), 0);
    CHECK_EQ(m.attr("double_or_zero")(42).cast<int>(), 84);

    CHECK_EQ(m.attr("half_or_none")(0).cast<int>(), nullptr);
}
#endif
