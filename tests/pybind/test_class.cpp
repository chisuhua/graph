...
TEST(class_, test_repr) {
    py::scoped_interpreter guard{};
    auto m = py::module::import("class_");
    auto pybindtest = py::module::import("pybind11_tests");
    auto UserType = pybindtest.attr("UserType");
    auto ConstructorStats = pybindtest.attr("ConstructorStats");

    py::print(py::dtype(UserType));
    py::print(UserType);

    auto a = m.attr("ProtectedA")();
    CHECK_EQ(a.attr("foo")().cast<int>(), 42);

    auto b = m.attr("ProtectedB")();
    CHECK_EQ(a.attr("foo")().cast<int>(), 42);
}
...
TEST(class_, test_instance) {
    ...
    */
    ...
    // auto i = m.attr("NoConstructor").attr("new_instance")(); // (cast<NoConstructor*>().new_instance();
}

PYBIND11_EMBEDDED_MODULE(class_, m) {
    // test_instance
