...
TEST(builtin_casters, test_tuple) {
    py::scoped_interpreter guard{};
    auto m = py::module::import("builtin_casters");
    CHECK_EQ(py::str(m.attr("pair_passthrough")(std::make_pair(true, "test"))).cast<std::string>(), R"(('test', True))");
    CHECK_EQ(py::str(m.attr("tuple_passthrough")(std::make_tuple(true, "test", 5))).cast<std::string>(), R"((5, 'test', True))");
}



PYBIND11_EMBEDDED_MODULE(builtin_casters, m) {
    // test_simple_string
