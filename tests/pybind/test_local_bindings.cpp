...
static void __main__ ()
{
    ...
    std::call_once(flag,
            []() {
                m = py::module::import("local_bindings");
                cm = py::module::import("pybind11_cross_module_tests");
            });

}

TEST(local_bindings, test_load_external) {
    py::scoped_interpreter guard{};
    // __main__();

    auto m = py::module::import("local_bindings");
    py::module cm = py::module::import("pybind11_cross_module_tests");
    CHECK_EQ(m.attr("load_external1")(cm.attr("ExternalType1")(11)).cast<int>(), 11);
    CHECK_EQ(m.attr("load_external2")(cm.attr("ExternalType2")(22)).cast<int>(), 22);
    try {
        m.attr("load_external2")(cm.attr("ExternalType1")(21)).cast<int>();
    } catch (const std::runtime_error& e) {
        std::cout << "go here " << e.what();
    }
}

TEST(local_bindings, test_local_bindings) {
    py::scoped_interpreter guard{};
    // __main__();
    // py::module m = py::module::import("local_bindings");
    // py::module cm = py::module::import("pybind11_cross_module_tests");
    //
    // auto i1 = m.attr("LocalType");
    // CHECK_EQ(i1.attr("get")().cast<int>(), 4);
    // CHECK_EQ(i1.attr("get3")().cast<int>(), 8);

    // auto i2 = cm.attr("LocalType")(10);
    // CHECK_EQ(i2.attr("get")().cast<int>(), 11);
    // CHECK_EQ(i2.attr("get2")().cast<int>(), 12);

    // CHECK_EQ(m.attr("local_value")(i1).cast<int>(), 5);
    // CHECK_EQ(cm.attr("local_value")(i2).cast<int>(), 5);

    try {
        // CHECK_EQ(m.attr("local_value")(i2).cast<int>(), 10);
        // CHECK_EQ(cm.attr("local_value")(i1).cast<int>(), 5);
    } catch (const std::runtime_error& e) {
        std::cout << "go here" << e.what();
    }
}

PYBIND11_EMBEDDED_MODULE(local_bindings, m) {
    // test_load_external
