...
TEST(numpy_array, test_dtypes)
{
    py::scoped_interpreter guard{};
    py::module m = py::module::import("numpy_array");
    py::module np = py::module::import("numpy");
    for( auto &size_check : get_platform_dtype_size_checks()) {
        py::print(py::cast(size_check));
        // CHECK_EQ(size_check.attr("size_cpp").cast<int>(), size_check.attr("size_numpy").cast<int>());
        CHECK_EQ(size_check.size_cpp, size_check.size_numpy);
    }

    // for( auto &check : m.attr("get_concrete_dtype_checks")()) {
    for( auto &check : get_concrete_dtype_checks()) {
        py::print(check);
        py::print(py::cast(check));
        // py::print(check.attr("numpy").attr("num"));
        // CHECK_EQ(check.attr("numpy").attr("num").cast<int>(), check.attr("pybind11").attr("num").cast<int>());
        CHECK_EQ(check.numpy.attr("num").cast<int>(), check.pybind11.attr("num").cast<int>());
        py::print("dypte attr num is {}"_s.format(check.numpy.attr("num").cast<int>()));

        CHECK_EQ(check.numpy.itemsize(), check.pybind11.itemsize());
        py::print("dypte itemsize num is {}"_s.format(check.numpy.itemsize()));
    }
}

TEST(numpy_array, test_array_attrbutes)
{
    // py::scoped_interpreter guard{};

    auto a = py::array(py::dtype("f8"), {}, {});
    // assert all(m.shape(a) == [])
    // assert all(m.strides(a) == [])
    // py::vectorize([](float a){ }py::array(a.ndim(), a.shape());
    CHECK_EQ(a.ndim(), 0);
    CHECK_EQ(a.writeable(), true);
    CHECK_EQ(a.size(), 1);
    CHECK_EQ(a.itemsize(), 8);
    CHECK_EQ(a.nbytes(), 8);
    CHECK_EQ(a.owndata(), true);

    std::vector<int32_t> data {1, 2, 3, 4, 5, 6};
    std::vector<ssize_t> shape { 3, 2 };
    std::vector<ssize_t> strides { 8, 4 };
    // auto dtype = py::dtype("unsigne short");

    // auto b = py::array(shape, data.data());
    // auto b = py::array(shape, strides, data.data());
}


PYBIND11_EMBEDDED_MODULE(numpy_array, sm) {
