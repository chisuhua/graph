 ...
 */
    ...
    struct NodeTypeChecker {
        ...
        static inline void PrintName(std::ostringstream& os)
        { // NOLINT(*)
            using ContainerType = typename T::ContainerType;
    ...
    struct NodeTypeChecker<Map<std::string, V>> {
        static inline bool Check(Node* sptr)
        {
            if (sptr == nullptr)
                return true;
            if (!sptr->is_type<StrMapNode>())
                return false;
            StrMapNode* n = static_cast<StrMapNode*>(sptr);
            for (const auto& kv : n->data) {
                if (!NodeTypeChecker<V>::Check(kv.second.get()))
                    return false;
            }
            return true;
        }
    ...
    struct NodeTypeChecker<Map<K, V>> {
        static inline bool Check(Node* sptr)
        {
            ...
            for (const auto& kv : n->data) {
                if (!NodeTypeChecker<K>::Check(kv.first.get()))
                    return false;
                if (!NodeTypeChecker<V>::Check(kv.second.get()))
                    return false;
            }
    ...
    inline void TVMArgsSetter::operator()(size_t i, const NodeRef& other) const
    { // NOLINT(*)
        if (other.defined()) {
