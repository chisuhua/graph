...
public:
    ...
    {
        return std::hash<int>()(
            static_cast<int>(value.code()) | (static_cast<int>(value.bits()) << 8) | (static_cast<int>(value.lanes()) << 16));
    }
    ...
    public:
        ...
        AttrNopEntry operator()(const char* key, T* lhs_value)
        {
            ...
            const T* rhs_value = reinterpret_cast<const T*>(
                reinterpret_cast<const char*>(rhs_) + (reinterpret_cast<const char*>(lhs_value) - reinterpret_cast<const char*>(lhs_)));
...
public:
    ...
    bool ContentEqual(const Node* other, AttrsEqual equal) const final
    {
        ...
        if (other == nullptr)
            return false;
        if (pself->type_index() != other->type_index())
            return false;
...
inline void BaseAttrsNode::PrintDocString(std::ostream& os) const
{ // NOLINT(*)
