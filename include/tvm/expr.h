...
public:
    Expr() {}
    explicit Expr(NodePtr<Node> ptr)
        : NodeRef(ptr)
    {
    }
    /*!
...
public:
   ...
   */
    explicit Integer(NodePtr<Node> node)
        : Expr(node)
    {
    }
    /*!
...
public:
    ...
    /*! \brief constructor */
    RangeNode() {}
    RangeNode(Expr min, Expr extent)
        : min(min)
        , extent(extent)
    {
    }
...
public:
    ...
    explicit IterVar(NodePtr<Node> n)
        : NodeRef(n)
    {
    }
    /*!
...
inline std::ostream& operator<<(std::ostream& os, const NodeRef& n)
{ // NOLINT(*)
