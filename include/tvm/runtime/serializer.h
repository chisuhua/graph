...
namespace serializer {
    ...
    struct Handler<DLDataType> {
        ...
        {
            if (!Handler<uint8_t>::Read(strm, &(dtype->code)))
                return false;
            if (!Handler<uint8_t>::Read(strm, &(dtype->bits)))
                return false;
            if (!Handler<uint16_t>::Read(strm, &(dtype->lanes)))
                return false;
            return true;
        }
    };
    ...
    struct Handler<DLContext> {
        ...
        inline static bool Read(Stream* strm, DLContext* ctx)
        {
            ...
            if (!Handler<int32_t>::Read(strm, &(device_type)))
                return false;
            ctx->device_type = static_cast<DLDeviceType>(device_type);
            if (!Handler<int32_t>::Read(strm, &(ctx->device_id)))
                return false;
            return true;
        }
    };
