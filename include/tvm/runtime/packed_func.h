 ...
 */
   ...
   */
        explicit PackedFunc(FType body)
            : body_(body)
        {
        }
        /*!
 ...
 */
   ...
   */
        ...
        const TVMValue& value() const
        {
            CHECK(type_code_ != kNodeHandle && type_code_ != kFuncHandle && type_code_ != kModuleHandle && type_code_ != kStr) << "TVMRetValue.value can only be used for POD data";
            return value_;
        }
...
#ifndef _LIBCPP_SGX_NO_IOSTREAMS
    inline std::ostream& operator<<(std::ostream& os, TVMType t)
    { // NOLINT(*)
    ...
    public:
        ...
        void operator()(size_t i, const Module& value) const
        { // NOLINT(*)
            values_[i].v_handle = const_cast<Module*>(&value);
            type_codes_[i] = kModuleHandle;
        }
        void operator()(size_t i, const NDArray& value) const
        { // NOLINT(*)
    ...
    namespace detail {
        ...
        struct unpack_call_dispatcher {
            ...
            static void run(const F& f,
                ...
                Args&&... unpacked_args)
            {
                unpack_call_dispatcher<R, nleft - 1, index + 1, F>::run(f, args_pack, rv,
