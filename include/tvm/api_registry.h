...
public:
    EnvFunc() {}
    explicit EnvFunc(NodePtr<Node> n)
        : NodeRef(n)
    {
    }
...
public:
    ...
    TypedEnvFunc() {}
    explicit TypedEnvFunc(NodePtr<Node> n)
        : NodeRef(n)
    {
    }
    /*!
   ...
   */
    R operator()(Args... args) const
    {
        const EnvFuncNode* n = operator->();
        CHECK(n != nullptr);
        return runtime::detail::typed_packed_call_dispatcher<R>::run(n->func, std::forward<Args>(args)...);
    }
    /*! \brief specify container node */
    using ContainerType = EnvFuncNode;
};
