...
#define TVM_DEFINE_NODE_REF_METHODS(TypeName, BaseTypeName, NodeName) \
    TypeName() {}                                                     \
    explicit TypeName(::tvm::NodePtr<::tvm::Node> n)                  \
        : BaseTypeName(n)                                             \
    {                                                                 \
    }                                                                 \
...
struct NodeFactoryReg {
   ...
   */
    ...
    // setter of creator
    NodeFactoryReg& set_creator(FCreate f)
    { // NOLINT(*)
        this->fcreator = f;
        return *this;
    }
    // setter of creator
    NodeFactoryReg& set_global_key(FGlobalKey f)
    { // NOLINT(*)
        this->fglobal_key = f;
        return *this;
    }
...
#define TVM_REGISTER_NODE_TYPE(TypeName) \
    static DMLC_ATTRIBUTE_UNUSED ::tvm::NodeFactoryReg& __make_Node##_##TypeName##__ = ::tvm::NodeFactoryReg::Registry()->__REGISTER__(TypeName::_type_key).set_creator([](const std::string&) { return ::tvm::make_node<TypeName>(); })
