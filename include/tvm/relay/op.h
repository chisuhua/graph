   ...
   */
    ...
    private:
        ...
        bool IsPrimitiveOp_() const
        {
            ...
            if (fn_ty->type_constraints.size() != 1)
                return false;
            ...
            for (size_t i = 0; i < fn_ty->type_params.size(); ++i) {
                if (!fn_ty->type_params[i].same_as(rel->args[i]))
                    return false;
            }
            return true;
        }
...
#define RELAY_REGISTER_OP(OpName)                                                               \
    DMLC_STR_CONCAT(RELAY_REGISTER_VAR_DEF, __COUNTER__) = ::tvm::relay::OpRegistry::Registry() \
            ...
            type_rel_func)
    {
        ...
        TypeConstraint type_rel = TypeRelationNode::make(env_type_rel_func,
            ty_call_args,
            arg_types.size(),
            Attrs());
    ...
    inline OpRegistry& OpRegistry::set_num_inputs(int32_t n)
    { // NOLINT(*)
        get()->num_inputs = n;
        return *this;
    }
    ...
    inline OpRegistry& OpRegistry::set_support_level(int32_t n)
    { // NOLINT(*)
        get()->support_level = n;
        return *this;
    }
