...
public:
   ...
   */
    ...
    {
        uint32_t type_index = n.type_index();
        CHECK(type_index < func_.size() && func_[type_index] != nullptr)
   ...
   */
    ...
    inline TSelf& set_dispatch(Function f)
    { // NOLINT(*)
   ...
   */
    template <typename TNode>
    inline TSelf& set_dispatch(std::function<R(const TNode* n, Args...)> f)
    { // NOLINT(*)
...
#define TVM_STATIC_IR_FUNCTOR_REGISTER(ClsName, FField) \
    TVM_STR_CONCAT(TVM_AUTO_REGISTER_VAR_DEF(ClsName), __COUNTER__) = MakeIRFunctorStaticRegistry(&ClsName::FField())
