...
#define LOG_STMT_MUTATIONS 0
    ...
    namespace {
        ...
        bool is_var_simple_const_comparison(const Expr& e, const Variable** ret)
        {
            ...
            return is_var_relop_simple_const<EQ>(e, ret) || is_var_relop_simple_const<NE>(e, ret);
        }
        ...
        {
            return propagate_indeterminate_expression(e0, t, expr) || propagate_indeterminate_expression(e1, t, expr);
        }

        bool propagate_indeterminate_expression(const Expr& e0, const Expr& e1, const Expr& e2, Type t, Expr* expr)
        {
            return propagate_indeterminate_expression(e0, t, expr) || propagate_indeterminate_expression(e1, t, expr) || propagate_indeterminate_expression(e2, t, expr);
        }
 ...
 * points in the code. */
    ...
    public:
            ...
            : simplify_lets(r)
        {
            ...
            for (Scope<Interval>::const_iterator iter = bi->cbegin(); iter != bi->cend(); ++iter) {
                int64_t i_min, i_max;
                if (const_int(iter.value().min, &i_min) && const_int(iter.value().max, &i_max)) {
    ...
    private:
            ...
            if (const int64_t* i = as_const_int(e)) {
                *min_val = *max_val = *i;
                ...
                if (const Min* min = max->a.as<Min>()) {
                    ...
                    // reordered x and a.
                    if (const_int_bounds(max->b, &min_b, &max_b) && (const_int_bounds(min->b, &min_a, &max_a) || const_int_bounds(min->a, &min_a, &max_a))) {
                        *min_val = min_b;
                        *max_val = std::max(max_a, max_b);
                        return true;
                    }
                } else if (const_int_bounds(max->a, &min_a, &max_a) && const_int_bounds(max->b, &min_b, &max_b)) {
                ...
                if (const Max* max = min->a.as<Max>()) {
                    ...
                    // a min/max clamp instead of a constant, simplify would have
                    // reordered x and a.
                    if (const_int_bounds(min->b, &min_b, &max_b) && (const_int_bounds(max->b, &min_a, &max_a) || const_int_bounds(max->a, &min_a, &max_a))) {
                        *min_val = std::min(min_a, min_b);
                        *max_val = max_b;
                        return true;
                    }
                } else if (const_int_bounds(min->a, &min_a, &max_a) && const_int_bounds(min->b, &min_b, &max_b)) {
                    *min_val = std::min(min_a, min_b);
                ...
                int64_t min_a, min_b, max_a, max_b;
                if (const_int_bounds(sel->true_value, &min_a, &max_a) && const_int_bounds(sel->false_value, &min_b, &max_b)) {
                    *min_val = std::min(min_a, min_b);
                    *max_val = std::max(max_a, max_b);
                    return true;
                }
                ...
                int64_t min_a, min_b, max_a, max_b;
                if (const_int_bounds(add->a, &min_a, &max_a) && const_int_bounds(add->b, &min_b, &max_b)) {
                    *min_val = min_a + min_b;
                    *max_val = max_a + max_b;
                    return no_overflow_scalar_int(t.element_of()) || (t.can_represent(*min_val) && t.can_represent(*max_val));
                }
            } else if (const Sub* sub = e.as<Sub>()) {
                int64_t min_a, min_b, max_a, max_b;
                if (const_int_bounds(sub->a, &min_a, &max_a) && const_int_bounds(sub->b, &min_b, &max_b)) {
                    *min_val = min_a - max_b;
                    *max_val = max_a - min_b;
                    return no_overflow_scalar_int(t.element_of()) || (t.can_represent(*min_val) && t.can_represent(*max_val));
                }
                ...
                int64_t min_a, min_b, max_a, max_b;
                if (const_int_bounds(mul->a, &min_a, &max_a) && const_int_bounds(mul->b, &min_b, &max_b)) {
                    int64_t
                        t0
                        = min_a * min_b,
                        t1 = min_a * max_b,
                ...
                int64_t min_b, max_b;
                if (const_int_bounds(mod->b, &min_b, &max_b) && (min_b > 0 || max_b < 0)) {
                    *min_val = 0;
                ...
                int64_t min_a, min_b, max_a, max_b;
                if (const_int_bounds(div->a, &min_a, &max_a) && const_int_bounds(div->b, &min_b, &max_b) && (min_b > 0 || max_b < 0)) {
                    int64_t
                        t0
                        = div_imp(min_a, min_b),
                ...
                if (const_int_bounds(r->base, &min_base, &max_base) && const_int_bounds(r->stride, &min_stride, &max_stride)) {
        ...
        {
            if (!no_overflow(e.type()))
                return Expr();
            const Div* div = e.as<Div>();
            if (!div)
                return Expr();
            if (!is_const(div->b, factor))
                return Expr();
            const Add* add = div->a.as<Add>();
            if (!add)
                return Expr();
            if (!is_const(add->b, factor - 1))
                return Expr();
            return add->a;
        }
        ...
        void visit(const Cast* op, const Expr& self)
        {
            ...
            if (value.type() == op->type) {
            ...
            } else if (cast && (op->type.is_int() || op->type.is_uint()) && (cast->type.is_int() || cast->type.is_uint()) && op->type.bits() <= cast->type.bits() && op->type.bits() <= op->value.type().bits()) {
            ...
            } else if (ramp_value && op->type.element_of() == Int(64) && op->value.type().element_of() == Int(32)) {
                ...
                expr = mutate(Ramp::make(Cast::make(op->type.element_of(), ramp_value->base),
                    ...
                    ramp_value->lanes));
            } else if (add && op->type == Int(64) && op->value.type() == Int(32) && is_const(add->b)) {
        ...
        void visit(const Add* op, const Expr& self)
        {
            ...
            // to check later.
            if ((is_simple_const(a) && !is_simple_const(b)) || (b.as<Min>() && !a.as<Min>()) || (b.as<Max>() && !a.as<Max>())) {
                std::swap(a, b);
            }
            ...
            if (const_int(a, &ia) && const_int(b, &ib)) {
                if (no_overflow(a.type()) && add_would_overflow(a.type().bits(), ia, ib)) {
            ...
            } else if (call_a && call_a->is_intrinsic(Call::signed_integer_overflow)) {
                expr = a;
            } else if (call_b && call_b->is_intrinsic(Call::signed_integer_overflow)) {
                expr = b;
            } else if (shuffle_a && shuffle_b && shuffle_a->is_slice() && shuffle_b->is_slice()) {
            ...
            } else if (select_a && select_b && equal(select_a->condition, select_b->condition)) {
            ...
            } else if (select_a && is_simple_const(b) && (is_simple_const(select_a->true_value) || is_simple_const(select_a->false_value))) {
            ...
            } else if (mul_b && !is_const(a) && equal(a, mul_b->a) && no_overflow(op->type)) {
                // a + a*b -> a*(1 + b)
                expr = mutate(a * (make_one(op->type) + mul_b->b));
            } else if (mul_b && !is_const(a) && equal(a, mul_b->b) && no_overflow(op->type)) {
                // a + b*a -> (1 + b)*a
                expr = mutate((make_one(op->type) + mul_b->a) * a);
            } else if (mul_a && !is_const(b) && equal(mul_a->a, b) && no_overflow(op->type)) {
                // a*b + a -> a*(b + 1)
                expr = mutate(mul_a->a * (mul_a->b + make_one(op->type)));
            } else if (mul_a && !is_const(b) && equal(mul_a->b, b) && no_overflow(op->type)) {
                // a*b + b -> (a + 1)*b
                expr = mutate((mul_a->a + make_one(op->type)) * b);
            } else if (no_overflow(op->type) && min_a && sub_a_b && equal(sub_a_b->b, b)) {
                // min(a, b-c) + c -> min(a+c, b)
                expr = mutate(Min::make(Add::make(min_a->a, b), sub_a_b->a));
            } else if (no_overflow(op->type) && min_a && sub_a_a && equal(sub_a_a->b, b)) {
                // min(a-c, b) + c -> min(a, b+c)
                expr = mutate(Min::make(sub_a_a->a, Add::make(min_a->b, b)));
            } else if (no_overflow(op->type) && max_a && sub_a_b && equal(sub_a_b->b, b)) {
                // max(a, b-c) + c -> max(a+c, b)
                expr = mutate(Max::make(Add::make(max_a->a, b), sub_a_b->a));
            } else if (no_overflow(op->type) && max_a && sub_a_a && equal(sub_a_a->b, b)) {
                // max(a-c, b) + c -> max(a, b+c)
                expr = mutate(Max::make(sub_a_a->a, Add::make(max_a->b, b)));

            } else if (no_overflow(op->type) && min_a && add_a_b && const_int(add_a_b->b, &ia) && const_int(b, &ib) && ia + ib == 0) {
                // min(a, b + (-2)) + 2 -> min(a + 2, b)
                expr = mutate(Min::make(Add::make(min_a->a, b), add_a_b->a));
            } else if (no_overflow(op->type) && min_a && add_a_a && const_int(add_a_a->b, &ia) && const_int(b, &ib) && ia + ib == 0) {
                // min(a + (-2), b) + 2 -> min(a, b + 2)
                expr = mutate(Min::make(add_a_a->a, Add::make(min_a->b, b)));
            } else if (no_overflow(op->type) && max_a && add_a_b && const_int(add_a_b->b, &ia) && const_int(b, &ib) && ia + ib == 0) {
                // max(a, b + (-2)) + 2 -> max(a + 2, b)
                expr = mutate(Max::make(Add::make(max_a->a, b), add_a_b->a));
            } else if (no_overflow(op->type) && max_a && add_a_a && const_int(add_a_a->b, &ia) && const_int(b, &ib) && ia + ib == 0) {
                // max(a + (-2), b) + 2 -> max(a, b + 2)
                expr = mutate(Max::make(add_a_a->a, Add::make(max_a->b, b)));
            } else if (min_a && max_b && equal(min_a->a, max_b->a) && equal(min_a->b, max_b->b)) {
                // min(x, y) + max(x, y) -> x + y
                expr = mutate(min_a->a + min_a->b);
            } else if (min_a && max_b && equal(min_a->a, max_b->b) && equal(min_a->b, max_b->a)) {
                // min(x, y) + max(y, x) -> x + y
                expr = mutate(min_a->a + min_a->b);
            } else if (no_overflow(op->type) && div_a && add_a_a && const_int(add_a_a->b, &ia) && const_int(div_a->b, &ib) && ib && const_int(b, &ic) &&
                ...
                // (x%3) + y*3 -> y*3 + x%3
                expr = mutate(b + a);
            } else if (no_overflow(op->type) && mul_a && mod_b && div_a_a && equal(mul_a->b, div_a_a->b) && equal(mul_a->b, mod_b->b) && equal(div_a_a->a, mod_b->a)) {
                // (x/3)*3 + x%3 -> x
                expr = div_a_a->a;
            } else if (no_overflow(op->type) && add_a && mul_a_a && mod_b && equal(mul_a_a->b, mod_b->b) && (!mod_a_b || !equal(mod_a_b->b, mod_b->b))) {
                // ((x*3) + y) + z%3 -> (x*3 + z%3) + y
                expr = mutate((add_a->a + b) + add_a->b);
            } else if (no_overflow(op->type) && add_a && mod_a_a && mul_b && equal(mod_a_a->b, mul_b->b) && (!mod_a_b || !equal(mod_a_b->b, mul_b->b))) {
                // ((x%3) + y) + z*3 -> (z*3 + x%3) + y
                expr = mutate((b + add_a->a) + add_a->b);
            } else if (no_overflow(op->type) && add_a && mul_a_b && mod_b && equal(mul_a_b->b, mod_b->b) && (!mod_a_a || !equal(mod_a_a->b, mod_b->b))) {
                // (y + (x*3)) + z%3 -> y + (x*3 + z%3)
                expr = mutate(add_a->a + (add_a->b + b));
            } else if (no_overflow(op->type) && add_a && mod_a_b && mul_b && equal(mod_a_b->b, mul_b->b) && (!mod_a_a || !equal(mod_a_a->b, mul_b->b))) {
                // (y + (x%3)) + z*3 -> y + (z*3 + x%3)
                expr = mutate(add_a->a + (b + add_a->b));
            } else if (mul_a && mul_b && const_int(mul_a->b, &ia) && const_int(mul_b->b, &ib) && ((ia % ib == 0) || (ib % ia == 0))) {
                // x*4 + y*2 -> (x*2 + y)*2
                // x*2 + y*4 -> (x + y*2)*2
                if (ia % ib == 0) {
        ...
        void visit(const Sub* op, const Expr& self)
        {
            ...
            if (is_zero(b)) {
                ...
                if (no_overflow(a.type()) && sub_would_overflow(a.type().bits(), ia, ib)) {
                    expr = signed_integer_overflow_error(a.type());
                } else {
                    expr = IntImm::make(a.type(), ia - ib);
                }
            ...
            } else if (max_a && equal(max_a->a, b) && !is_const(b) && no_overflow(op->type)) {
                // max(a, b) - a -> max(0, b-a)
                expr = mutate(Max::make(make_zero(op->type), max_a->b - max_a->a));
            } else if (min_a && equal(min_a->a, b) && !is_const(b) && no_overflow(op->type)) {
                // min(a, b) - a -> min(0, b-a)
                expr = mutate(Min::make(make_zero(op->type), min_a->b - min_a->a));
            } else if (max_a && equal(max_a->b, b) && !is_const(b) && no_overflow(op->type)) {
                // max(a, b) - b -> max(a-b, 0)
                expr = mutate(Max::make(max_a->a - max_a->b, make_zero(op->type)));
            } else if (min_a && equal(min_a->b, b) && !is_const(b) && no_overflow(op->type)) {
                // min(a, b) - b -> min(a-b, 0)
                expr = mutate(Min::make(min_a->a - min_a->b, make_zero(op->type)));

            } else if (max_b && equal(max_b->a, a) && !is_const(a) && no_overflow(op->type)) {
                // a - max(a, b) -> 0 - max(0, b-a) -> min(0, a-b)
                expr = mutate(Min::make(make_zero(op->type), max_b->a - max_b->b));
            } else if (min_b && equal(min_b->a, a) && !is_const(a) && no_overflow(op->type)) {
                // a - min(a, b) -> 0 - min(0, b-a) -> max(0, a-b)
                expr = mutate(Max::make(make_zero(op->type), min_b->a - min_b->b));
            } else if (max_b && equal(max_b->b, a) && !is_const(a) && no_overflow(op->type)) {
                // b - max(a, b) -> 0 - max(a-b, 0) -> min(b-a, 0)
                expr = mutate(Min::make(max_b->b - max_b->a, make_zero(op->type)));
            } else if (min_b && equal(min_b->b, a) && !is_const(a) && no_overflow(op->type)) {
            ...
            } else if (no_overflow(op->type) && min_b && add_b_a && no_overflow(op->type) && equal(a, add_b_a->a)) {
            ...
            } else if (no_overflow(op->type) && min_b && add_b_a && no_overflow(op->type) && equal(a, add_b_a->b)) {
                // a - min(b + a, c) -> max(-b, a-c)
                expr = mutate(max(0 - add_b_a->a, a - min_b->b));
            } else if (no_overflow(op->type) && min_b && add_b_b && equal(a, add_b_b->a)) {
                // a - min(c, a + b) -> max(-b, a-c)
                expr = mutate(max(0 - add_b_b->b, a - min_b->a));
            } else if (no_overflow(op->type) && min_b && add_b_b && equal(a, add_b_b->b)) {
                // a - min(c, b + a) -> max(-b, a-c)
                expr = mutate(max(0 - add_b_b->a, a - min_b->a));
            } else if (no_overflow(op->type) && min_a && add_a_a && equal(b, add_a_a->a)) {
                // min(a + b, c) - a -> min(b, c-a)
                expr = mutate(min(add_a_a->b, min_a->b - b));
            } else if (no_overflow(op->type) && min_a && add_a_a && equal(b, add_a_a->b)) {
                // min(b + a, c) - a -> min(b, c-a)
                expr = mutate(min(add_a_a->a, min_a->b - b));
            } else if (no_overflow(op->type) && min_a && add_a_b && equal(b, add_a_b->a)) {
                // min(c, a + b) - a -> min(b, c-a)
                expr = mutate(min(add_a_b->b, min_a->a - b));
            } else if (no_overflow(op->type) && min_a && add_a_b && equal(b, add_a_b->b)) {
                ...
                // max(a, b) - max(b, a) -> 0
                expr = make_zero(op->type);
            } else if (no_overflow(op->type) && min_a && min_b && is_zero(mutate((min_a->a + min_b->b) - (min_a->b + min_b->a)))) {
                // min(a, b) - min(c, d) where a-b == c-d -> b - d
                expr = mutate(min_a->b - min_b->b);
            } else if (no_overflow(op->type) && max_a && max_b && is_zero(mutate((max_a->a + max_b->b) - (max_a->b + max_b->a)))) {
                // max(a, b) - max(c, d) where a-b == c-d -> b - d
                expr = mutate(max_a->b - max_b->b);
            } else if (no_overflow(op->type) && min_a && min_b && is_zero(mutate((min_a->a + min_b->a) - (min_a->b + min_b->b)))) {
                // min(a, b) - min(c, d) where a-b == d-c -> b - c
                expr = mutate(min_a->b - min_b->a);
            } else if (no_overflow(op->type) && max_a && max_b && is_zero(mutate((max_a->a + max_b->a) - (max_a->b + max_b->b)))) {
                // max(a, b) - max(c, d) where a-b == d-c -> b - c
                expr = mutate(max_a->b - max_b->a);
            } else if (no_overflow(op->type) && (op->type.is_int() || op->type.is_uint()) && mul_a && div_a_a && is_positive_const(mul_a->b) && equal(mul_a->b, div_a_a->b) && equal(div_a_a->a, b)) {
                // (x/4)*4 - x -> -(x%4)
                expr = mutate(make_zero(a.type()) - (b % mul_a->b));
            } else if (no_overflow(op->type) && (op->type.is_int() || op->type.is_uint()) && mul_b && div_b_a && is_positive_const(mul_b->b) && equal(mul_b->b, div_b_a->b) && equal(div_b_a->a, a)) {
                // x - (x/4)*4 -> x%4
                expr = mutate(a % mul_b->b);
            } else if (div_a && div_b && is_positive_const(div_a->b) && equal(div_a->b, div_b->b) && op->type.is_int() && no_overflow(op->type) && add_a_a && add_b_a && equal(add_a_a->a, add_b_a->a) && (is_simple_const(add_a_a->b) || is_simple_const(add_b_a->b))) {
            ...
            } else if (div_a && div_b && is_positive_const(div_a->b) && equal(div_a->b, div_b->b) && op->type.is_int() && no_overflow(op->type) && add_b_a && equal(div_a->a, add_b_a->a)) {
                // Same as above, where a == 0
                Expr x = div_a->a, b = add_b_a->b, c = div_a->b;
                expr = mutate(((c - 1 - b) - (x % c)) / c);
            } else if (div_a && div_b && is_positive_const(div_a->b) && equal(div_a->b, div_b->b) && op->type.is_int() && no_overflow(op->type) && add_a_a && equal(add_a_a->a, div_b->a)) {
                // Same as above, where b == 0
                Expr x = add_a_a->a, a = add_a_a->b, c = div_a->b;
                expr = mutate(((x % c) + a) / c);
            } else if (div_a && div_b && is_positive_const(div_a->b) && equal(div_a->b, div_b->b) && op->type.is_int() && no_overflow(op->type) && sub_b_a && equal(div_a->a, sub_b_a->a)) {
                // Same as above, where a == 0 and b is subtracted
                Expr x = div_a->a, b = sub_b_a->b, c = div_a->b;
                expr = mutate(((c - 1 + b) - (x % c)) / c);
            } else if (div_a && div_b && is_positive_const(div_a->b) && equal(div_a->b, div_b->b) && op->type.is_int() && no_overflow(op->type) && sub_a_a && equal(sub_a_a->a, div_b->a)) {
                // Same as above, where b == 0, and a is subtracted
                Expr x = sub_a_a->a, a = sub_a_a->b, c = div_a->b;
                expr = mutate(((x % c) - a) / c);
            } else if (div_a && div_b && is_positive_const(div_a->b) && equal(div_a->b, div_b->b) && op->type.is_int() && no_overflow(op->type) && sub_a_a && add_b_a && equal(sub_a_a->a, add_b_a->a) && is_simple_const(add_b_a->b)) {
                ...
                expr = mutate((((x + (b % c)) % c) - a - b) / c);
            } else if (div_a && div_b && is_positive_const(div_a->b) && equal(div_a->b, div_b->b) && op->type.is_int() && no_overflow(op->type) && add_a_a && sub_b_a && equal(add_a_a->a, sub_b_a->a) && is_simple_const(add_a_a->b)) {
        ...
        void visit(const Mul* op, const Expr& self)
        {
            ...
            if (is_simple_const(a) || (b.as<Min>() && a.as<Max>())) {
                std::swap(a, b);
            }
            ...
            if (is_zero(a)) {
                ...
                if (no_overflow(a.type()) && mul_would_overflow(a.type().bits(), ia, ib)) {
                    expr = signed_integer_overflow_error(a.type());
                } else {
                    expr = IntImm::make(a.type(), ia * ib);
                }
        ...
        void visit(const Div* op, const Expr& self)
        {
            ...
            if (const_int(b, &ib) && ib && const_int_bounds(a, &num_min, &num_max) && div_imp(num_max, ib) == div_imp(num_min, ib)) {
            ...
            } else if (no_overflow_scalar_int(op->type) && is_const(a, -1)) {
                // -1/x -> select(x < 0, 1, -1)
                expr = mutate(select(b < make_zero(op->type),
                    make_one(op->type),
                    make_const(op->type, -1)));
            } else if (ramp_a && no_overflow_scalar_int(ramp_a->base.type()) && const_int(ramp_a->stride, &ia) && broadcast_b && const_int(broadcast_b->value, &ib) && ib && ia % ib == 0) {
                // ramp(x, 4, w) / broadcast(2, w) -> ramp(x / 2, 2, w)
                Type t = op->type.element_of();
                expr = mutate(Ramp::make(ramp_a->base / broadcast_b->value,
                    IntImm::make(t, div_imp(ia, ib)),
                    ramp_a->lanes));
            } else if (ramp_a && no_overflow_scalar_int(ramp_a->base.type()) && const_int(ramp_a->stride, &ia) && broadcast_b && const_int(broadcast_b->value, &ib) && ib != 0 && mod_rem.modulus % ib == 0 && div_imp((int64_t)mod_rem.remainder, ib) == div_imp(mod_rem.remainder + (ramp_a->lanes - 1) * ia, ib)) {
                // ramp(k*z + x, y, w) / z = broadcast(k, w) if x/z == (x + (w-1)*y)/z
                expr = mutate(Broadcast::make(ramp_a->base / broadcast_b->value, ramp_a->lanes));
            } else if (no_overflow(op->type) && div_a && const_int(div_a->b, &ia) && ia >= 0 && const_int(b, &ib) && ib >= 0) {
                // (x / 3) / 4 -> x / 12
                expr = mutate(div_a->a / make_const(op->type, ia * ib));
            } else if (no_overflow(op->type) && div_a_a && add_a && const_int(div_a_a->b, &ia) && ia >= 0 && const_int(add_a->b, &ib) && const_int(b, &ic) && ic >= 0) {
                // (x / ia + ib) / ic -> (x + ia*ib) / (ia*ic)
                expr = mutate((div_a_a->a + make_const(op->type, ia * ib)) / make_const(op->type, ia * ic));
            } else if (no_overflow(op->type) && mul_a && const_int(mul_a->b, &ia) && const_int(b, &ib) && ia > 0 && ib > 0 && (ia % ib == 0 || ib % ia == 0)) {
                if (ia % ib == 0) {
                    // (x * 4) / 2 -> x * 2
            ...
            } else if (no_overflow(op->type) && (add_a || sub_a || max_a || min_a) && mul_a_a && const_int(mul_a_a->b, &ia) && const_int(b, &ib) && ib > 0 && (ia % ib == 0)) {
            ...
            } else if (no_overflow(op->type) && (add_a || sub_a || max_a || min_a) && mul_a_b && const_int(mul_a_b->b, &ia) && const_int(b, &ib) && ib > 0 && (ia % ib == 0)) {
                // Pull out terms that are a multiple of the divisor
                Expr ratio = make_const(op->type, div_imp(ia, ib));
                if (add_a) {
                    // (y + x*4) / 2 -> y/2 + x*2
            ...
            } else if (no_overflow(op->type) && add_a && add_a_a && mul_a_a_a && const_int(mul_a_a_a->b, &ia) && const_int(b, &ib) && ib > 0 && (ia % ib == 0)) {
                // Pull terms that are a multiple of the divisor out
                // ((x*4 + y) + z) / 2 -> x*2 + (y + z)/2
                Expr ratio = make_const(op->type, div_imp(ia, ib));
                expr = mutate((mul_a_a_a->a * ratio) + (add_a_a->b + add_a->b) / b);
            } else if (no_overflow(op->type) && add_a && sub_a_a && mul_a_a_a && const_int(mul_a_a_a->b, &ia) && const_int(b, &ib) && ib > 0 && (ia % ib == 0)) {
                // ((x*4 - y) + z) / 2 -> x*2 + (z - y)/2
                Expr ratio = make_const(op->type, div_imp(ia, ib));
                expr = mutate((mul_a_a_a->a * ratio) + (add_a->b - sub_a_a->b) / b);
            } else if (no_overflow(op->type) && sub_a && add_a_a && mul_a_a_a && const_int(mul_a_a_a->b, &ia) && const_int(b, &ib) && ib > 0 && (ia % ib == 0)) {
                // ((x*4 + y) - z) / 2 -> x*2 + (y - z)/2
                Expr ratio = make_const(op->type, div_imp(ia, ib));
                expr = mutate((mul_a_a_a->a * ratio) + (add_a_a->b - sub_a->b) / b);
            } else if (no_overflow(op->type) && sub_a && sub_a_a && mul_a_a_a && const_int(mul_a_a_a->b, &ia) && const_int(b, &ib) && ib > 0 && (ia % ib == 0)) {
                // ((x*4 - y) - z) / 2 -> x*2 + (0 - y - z)/2
                Expr ratio = make_const(op->type, div_imp(ia, ib));
                expr = mutate((mul_a_a_a->a * ratio) + (-sub_a_a->b - sub_a->b) / b);
            } else if (no_overflow(op->type) && add_a && add_a_b && mul_a_b_a && const_int(mul_a_b_a->b, &ia) && const_int(b, &ib) && ib > 0 && (ia % ib == 0)) {
                // (x + (y*4 + z)) / 2 -> y*2 + (x + z)/2
                Expr ratio = make_const(op->type, div_imp(ia, ib));
                expr = mutate((mul_a_b_a->a * ratio) + (add_a->a + add_a_b->b) / b);
            } else if (no_overflow(op->type) && add_a && sub_a_b && mul_a_b_a && const_int(mul_a_b_a->b, &ia) && const_int(b, &ib) && ib > 0 && (ia % ib == 0)) {
                // (x + (y*4 - z)) / 2 -> y*2 + (x - z)/2
                Expr ratio = make_const(op->type, div_imp(ia, ib));
                expr = mutate((mul_a_b_a->a * ratio) + (add_a->a - sub_a_b->b) / b);
            } else if (no_overflow(op->type) && sub_a && add_a_b && mul_a_b_a && const_int(mul_a_b_a->b, &ia) && const_int(b, &ib) && ib > 0 && (ia % ib == 0)) {
                // (x - (y*4 + z)) / 2 -> (x - z)/2 - y*2
                Expr ratio = make_const(op->type, div_imp(ia, ib));
                expr = mutate((sub_a->a - add_a_b->b) / b - (mul_a_b_a->a * ratio));
            } else if (no_overflow(op->type) && add_a && sub_a_b && mul_a_b_b && const_int(mul_a_b_b->b, &ia) && const_int(b, &ib) && ib > 0 && (ia % ib == 0)) {
                // (x - (z*4 - y)) / 2 -> (x + (y - z*4)) / 2  -- by a rule from Sub
                // (x + (y - z*4)) / 2 -> (x + y)/2 - z*2  -- by this rule
                Expr ratio = make_const(op->type, div_imp(ia, ib));
                expr = mutate((add_a->a + sub_a_b->a) / b - (mul_a_b_b->a * ratio));
            } else if (no_overflow(op->type) && add_a && const_int(add_a->b, &ia) && const_int(b, &ib) && ib > 0 && (ia % ib == 0)) {
                // (y + 8) / 2 -> y/2 + 4
                Expr ratio = make_const(op->type, div_imp(ia, ib));
            ...
            } else if (no_overflow(op->type) && sub_a && !is_zero(b) && equal(sub_a->a, b)) {
                // (x - y)/x -> (-y)/x + 1
                expr = mutate((make_zero(op->type) - sub_a->b) / b + make_one(op->type));
            } else if (no_overflow(op->type) && sub_a && equal(sub_a->b, b)) {
                // (y - x)/x -> y/x - 1
                expr = mutate(sub_a->a / b + make_const(op->type, -1));

            } else if (no_overflow(op->type) && add_a && add_a_a && equal(add_a_a->a, b)) {
                // ((x + y) + z)/x -> ((y + z) + x)/x -> (y+z)/x + 1
                expr = mutate((add_a_a->b + add_a->b) / b + make_one(op->type));
            } else if (no_overflow(op->type) && add_a && add_a_a && equal(add_a_a->b, b)) {
                // ((y + x) + z)/x -> ((y + z) + x)/x -> (y+z)/x + 1
                expr = mutate((add_a_a->a + add_a->b) / b + make_one(op->type));
            } else if (no_overflow(op->type) && add_a && add_a_b && equal(add_a_b->b, b)) {
                // (y + (z + x))/x -> ((y + z) + x)/x -> (y+z)/x + 1
                expr = mutate((add_a->a + add_a_b->a) / b + make_one(op->type));
            } else if (no_overflow(op->type) && add_a && add_a_b && equal(add_a_b->a, b)) {
                // (y + (x + z))/x -> ((y + z) + x)/x -> (y+z)/x + 1
                expr = mutate((add_a->a + add_a_b->b) / b + make_one(op->type));

            } else if (no_overflow(op->type) && mul_a && equal(mul_a->b, b)) {
                // (x*y)/y
                expr = mul_a->a;
            } else if (no_overflow(op->type) && mul_a && equal(mul_a->a, b)) {
                // (y*x)/y
                expr = mul_a->b;
            } else if (no_overflow(op->type) && add_a && mul_a_a && equal(mul_a_a->b, b)) {
                // (x*a + y) / a -> x + y/a
                expr = mutate(mul_a_a->a + (add_a->b / b));
            } else if (no_overflow(op->type) && add_a && mul_a_a && equal(mul_a_a->a, b)) {
                // (a*x + y) / a -> x + y/a
                expr = mutate(mul_a_a->b + (add_a->b / b));
            } else if (no_overflow(op->type) && add_a && mul_a_b && equal(mul_a_b->b, b)) {
                // (y + x*a) / a -> y/a + x
                expr = mutate((add_a->a / b) + mul_a_b->a);
            } else if (no_overflow(op->type) && add_a && mul_a_b && equal(mul_a_b->a, b)) {
                // (y + a*x) / a -> y/a + x
                expr = mutate((add_a->a / b) + mul_a_b->b);

            } else if (no_overflow(op->type) && sub_a && mul_a_a && equal(mul_a_a->b, b)) {
                // (x*a - y) / a -> x + (-y)/a
                expr = mutate(mul_a_a->a + ((make_zero(op->type) - sub_a->b) / b));
            } else if (no_overflow(op->type) && sub_a && mul_a_a && equal(mul_a_a->a, b)) {
                // (a*x - y) / a -> x + (-y)/a
                expr = mutate(mul_a_a->b + ((make_zero(op->type) - sub_a->b) / b));
            } else if (no_overflow(op->type) && sub_a && mul_a_b && equal(mul_a_b->b, b)) {
                // (y - x*a) / a -> y/a - x
                expr = mutate((sub_a->a / b) - mul_a_b->a);
            } else if (no_overflow(op->type) && sub_a && mul_a_b && equal(mul_a_b->a, b)) {
                // (y - a*x) / a -> y/a - x
        ...
        void visit(const Mod* op, const Expr& self)
        {
            ...
            if (const_int(b, &ib) && ib && no_overflow_scalar_int(op->type)) {
                ...
                int64_t a_min, a_max;
                if (const_int_bounds(a, &a_min, &a_max) && a_max < ib && a_min >= 0) {
                    expr = a;
                    return;
                }
            ...
            if (broadcast_b && const_int(broadcast_b->value, &ib) && ib && ramp_a && no_overflow_scalar_int(ramp_a->base.type())) {
                mod_rem = modulus_remainder(ramp_a->base, alignment_info);
            }
            ...
            } else if (no_overflow(op->type) && mul_a && const_int(b, &ib) && ib && const_int(mul_a->b, &ia) && (ia % ib == 0)) {
                // (x * (b*a)) % b -> 0
                expr = make_zero(op->type);
            } else if (no_overflow(op->type) && add_a && mul_a_a && const_int(mul_a_a->b, &ia) && const_int(b, &ib) && ib && (ia % ib == 0)) {
                // (x * (b*a) + y) % b -> (y % b)
            ...
            else if (no_overflow(op->type) && add_a && mul_a_b && const_int(mul_a_b->b, &ia) && const_int(b, &ib) && ib && (ia % ib == 0)) {
                // (y + x * (b*a)) % b -> (y % b)
                expr = mutate(add_a->a % b);
            } else if (no_overflow_scalar_int(op->type) && const_int(b, &ib) && ib && mod_rem.modulus % ib == 0) {
                // ((a*b)*x + c) % a -> c % a
                expr = make_const(op->type, mod_imp((int64_t)mod_rem.remainder, ib));
            } else if (no_overflow(op->type) && ramp_a && const_int(ramp_a->stride, &ia) && broadcast_b && const_int(broadcast_b->value, &ib) && ib && ia % ib == 0) {
            ...
            } else if (ramp_a && no_overflow_scalar_int(ramp_a->base.type()) && const_int(ramp_a->stride, &ia) && !is_const(ramp_a->base) && broadcast_b && const_int(broadcast_b->value, &ib) && ib != 0 && mod_rem.modulus % ib == 0) {
        ...
        void visit(const Min* op, const Expr& self)
        {
            ...
            if (equal(a, b)) {
                ...
                return;
            } else if (const_int_bounds(a, &a_min, &a_max) && const_int_bounds(b, &b_min, &b_max)) {
                if (a_min >= b_max) {
                    expr = b;
                    return;
                } else if (b_min >= a_max) {
            ...
            } else if (no_overflow(op->type) && ramp_a && broadcast_b && const_int_bounds(a, &ramp_min, &ramp_max) && const_int(broadcast_b->value, &ic)) {
            ...
            if (no_overflow(op->type) && add_a && const_int(add_a->b, &ia) && add_b && const_int(add_b->b, &ib) && equal(add_a->a, add_b->a)) {
                // min(x + 3, x - 2) -> x - 2
                if (ia > ib) {
                    expr = b;
                } else {
            ...
            } else if (no_overflow(op->type) && add_a && const_int(add_a->b, &ia) && equal(add_a->a, b)) {
                // min(x + 5, x) -> x
                if (ia > 0) {
                    expr = b;
                } else {
            ...
            } else if (no_overflow(op->type) && add_b && const_int(add_b->b, &ib) && equal(add_b->a, a)) {
                // min(x, x + 5) -> x
                if (ib > 0) {
                    expr = a;
                } else {
            ...
            } else if (no_overflow(op->type) && sub_a && sub_b && equal(sub_a->b, sub_b->b) && const_int(sub_a->a, &ia) && const_int(sub_b->a, &ib)) {
                ...
                // min(((a + 3)/4)*4, a) -> a
                expr = b;
            } else if (a_round_up.defined() && max_b && equal(a_round_up, max_b->a) && is_const(max_b->b, a_round_up_factor)) {
                // min(((a + 3)/4)*4, max(a, 4)) -> max(a, 4)
                expr = b;
            } else if (b_round_up.defined() && equal(b_round_up, a)) {
                // min(a, ((a + 3)/4)*4) -> a
                expr = a;
            } else if (b_round_up.defined() && max_a && equal(b_round_up, max_a->a) && is_const(max_a->b, b_round_up_factor)) {
                // min(max(a, 4), ((a + 3)/4)*4) -> max(a, 4)
                expr = a;
            } else if (max_a && min_b && equal(max_a->a, min_b->a) && equal(max_a->b, min_b->b)) {
                // min(max(x, y), min(x, y)) -> min(x, y)
                expr = mutate(min(max_a->a, max_a->b));
            } else if (max_a && min_b && equal(max_a->a, min_b->b) && equal(max_a->b, min_b->a)) {
                // min(max(x, y), min(y, x)) -> min(x, y)
                expr = mutate(min(max_a->a, max_a->b));
            } else if (max_a && (equal(max_a->a, b) || equal(max_a->b, b))) {
                // min(max(x, y), x) -> x
                // min(max(x, y), y) -> y
                expr = b;
                ...
                expr = b;
            } else if (min_a && broadcast_a_b && broadcast_b) {
            ...
            } else if (no_overflow(op->type) && div_a && div_b && const_int(div_a->b, &ia) && ia && const_int(div_b->b, &ib) && (ia == ib)) {
            ...
            } else if (no_overflow(op->type) && mul_a && mul_b && const_int(mul_a->b, &ia) && const_int(mul_b->b, &ib) && (ia == ib)) {
                Expr factor = make_const(op->type, ia);
                if (ia > 0) {
            ...
            } else if (no_overflow(op->type) && mul_a && const_int(mul_a->b, &ia) && const_int(b, &ib) && ia && (ib % ia == 0)) {
            ...
            } else if (call_a && call_a->is_intrinsic(Call::likely) && equal(call_a->args[0], b)) {
                // min(likely(b), b) -> likely(b)
                expr = a;
            } else if (call_b && call_b->is_intrinsic(Call::likely) && equal(call_b->args[0], a)) {
        ...
        void visit(const Max* op, const Expr& self)
        {
            ...
            if (no_overflow(op->type) && add_a && const_int(add_a->b, &ia) && add_b && const_int(add_b->b, &ib) && equal(add_a->a, add_b->a)) {
                ...
                expr = mutate(max(min_a->a, min_a->b));
            } else if (min_a && (equal(min_a->a, b) || equal(min_a->b, b))) {
                // max(min(x, y), x) -> x
                // max(min(x, y), y) -> y
                expr = b;
                ...
                expr = b;
            } else if (max_a && broadcast_a_b && broadcast_b) {
            ...
            } else if (max_a && max_a_a && equal(max_a_a->b, b)) {
            ...
            } else if (min_a && max_a_a && equal(max_a_a->b, b)) {
                // max(min(max(x, y), z), y) -> max(min(x, z), y)
                expr = mutate(max(min(max_a_a->a, min_a->b), b));
            } else if (min_a && max_a_a && equal(max_a_a->a, b)) {
            ...
            } else if (no_overflow(op->type) && sub_a && is_simple_const(sub_a->a) && is_simple_const(b)) {
                // max(8 - x, 3) -> 8 - min(x, 5)
        ...
        void visit(const EQ* op, const Expr& self)
        {
            ...
            if (is_zero(delta)) {
                ...
                if (const_int_bounds(delta, &delta_min, &delta_max) && (delta_min > 0 || delta_max < 0)) {
                    expr = const_false();
                    return;
                }
            }
            ...
            if (broadcast) {
                ...
                expr = Broadcast::make(mutate(broadcast->value == make_zero(broadcast->value.type())),
                    broadcast->lanes);
            ...
            } else if (mul && no_overflow(mul->type)) {
            ...
            } else if (sel && (is_positive_const(sel->true_value) || is_negative_const(sel->true_value))) {
                // select(c, 4, f) == 0 -> !c && (f == 0)
                expr = mutate((!sel->condition) && (sel->false_value == zero));
            } else if (sel && is_zero(sel->false_value)) {
                // select(c, t, 0) == 0 -> !c || (t == 0)
                expr = mutate((!sel->condition) || (sel->true_value == zero));
            } else if (sel && (is_positive_const(sel->false_value) || is_negative_const(sel->false_value))) {
        ...
        void visit(const LT* op, const Expr& self)
        {
            ...
            int64_t a_min, a_max, b_min, b_max;
            if (const_int_bounds(a, &a_min, &a_max) && const_int_bounds(b, &b_min, &b_max)) {
                if (a_max < b_min) {
                    expr = const_true(op->type.lanes());
                    return;
                }
            ...
            if (const_int(a, &ia) && const_int(b, &ib)) {
                ...
                if (ramp_a && ramp_b && equal(ramp_a->stride, ramp_b->stride)) {
                    ...
                    expr = mutate(make_zero(add_b->a.type()) < add_b->a);
                } else if (add_b && is_simple_const(a) && is_simple_const(add_b->b)) {
                    // a < x + b -> (a - b) < x
                    expr = mutate((a - add_b->b) < add_b->a);
                } else if (sub_b && equal(sub_b->a, a)) {
                    // Subtract a term from both sides
                    expr = mutate(sub_b->b < make_zero(sub_b->b.type()));
                } else if (sub_b && is_const(a) && is_const(sub_b->a) && !is_const(sub_b->b)) {
                    // (c1 < c2 - x) -> (x < c2 - c1)
                    expr = mutate(sub_b->b < (sub_b->a - a));
                } else if (mul_a && mul_b && is_positive_const(mul_a->b) && is_positive_const(mul_b->b) && equal(mul_a->b, mul_b->b)) {
                    // Divide both sides by a constant
                    expr = mutate(mul_a->a < mul_b->a);
                } else if (mul_a && is_positive_const(mul_a->b) && is_const(b)) {
                ...
                } else if (mul_b && is_positive_const(mul_b->b) && is_const(a)) {
                    // (c1 < b * c2) <=> ((c1 / c2) < b)
                    expr = mutate((a / mul_b->b) < mul_b->a);
                } else if (a.type().is_int() && div_a && is_positive_const(div_a->b) && is_const(b)) {
                    // a / c1 < c2 <=> a < c1*c2
                    expr = mutate(div_a->a < (div_a->b * b));
                } else if (a.type().is_int() && div_b && is_positive_const(div_b->b) && is_const(a)) {
                    // c1 < b / c2 <=> (c1+1)*c2-1 < b
                ...
                } else if (mul_a && div_a_a && const_int(div_a_a->b, &ia) && const_int(mul_a->b, &ib) && ia > 0 && ia == ib && equal(div_a_a->a, b)) {
                    // subtract (x/c1)*c1 from both sides
                    // (x/c1)*c1 < x -> 0 < x % c1
                    expr = mutate(0 < b % make_const(a.type(), ia));
                } else if (mul_a && div_a_a && add_b && const_int(div_a_a->b, &ia) && const_int(mul_a->b, &ib) && ia > 0 && ia == ib && equal(div_a_a->a, add_b->a)) {
                    // subtract (x/c1)*c1 from both sides
                    // (x/c1)*c1 < x + y -> 0 < x % c1 + y
                    expr = mutate(0 < add_b->a % div_a_a->b + add_b->b);
                } else if (mul_a && div_a_a && sub_b && const_int(div_a_a->b, &ia) && const_int(mul_a->b, &ib) && ia > 0 && ia == ib && equal(div_a_a->a, sub_b->a)) {
                    // subtract (x/c1)*c1 from both sides
                    // (x/c1)*c1 < x - y -> y < x % c1
                    expr = mutate(sub_b->b < sub_b->a % div_a_a->b);
                } else if (mul_a && div_a_a && add_a_a_a && const_int(div_a_a->b, &ia) && const_int(mul_a->b, &ib) && const_int(add_a_a_a->b, &ic) && ia > 0 && ia == ib && equal(add_a_a_a->a, b)) {
                    // subtract ((x+c2)/c1)*c1 from both sides
                    // ((x+c2)/c1)*c1 < x -> c2 < (x+c2) % c1
                    expr = mutate(add_a_a_a->b < div_a_a->a % div_a_a->b);
                } else if (mul_a && div_a_a && add_b && add_a_a_a && const_int(div_a_a->b, &ia) && const_int(mul_a->b, &ib) && const_int(add_a_a_a->b, &ic) && ia > 0 && ia == ib && equal(add_a_a_a->a, add_b->a)) {
                    // subtract ((x+c2)/c1)*c1 from both sides
                    // ((x+c2)/c1)*c1 < x + y -> c2 < (x+c2) % c1 + y
                    expr = mutate(add_a_a_a->b < div_a_a->a % div_a_a->b + add_b->b);
                } else if (mul_a && div_a_a && add_a_a_a && sub_b && const_int(div_a_a->b, &ia) && const_int(mul_a->b, &ib) && const_int(add_a_a_a->b, &ic) && ia > 0 && ia == ib && equal(add_a_a_a->a, sub_b->a)) {
                    // subtract ((x+c2)/c1)*c1 from both sides
                    // ((x+c2)/c1)*c1 < x - y -> y < (x+c2) % c1 + (-c2)
                    expr = mutate(sub_b->b < div_a_a->a % div_a_a->b + make_const(a.type(), -ic));
                } else if (delta_ramp && is_positive_const(delta_ramp->stride) && is_one(mutate(delta_ramp->base + delta_ramp->stride * (delta_ramp->lanes - 1) < 0))) {
                    expr = const_true(delta_ramp->lanes);
                } else if (delta_ramp && is_positive_const(delta_ramp->stride) && is_one(mutate(delta_ramp->base >= 0))) {
                    expr = const_false(delta_ramp->lanes);
                } else if (delta_ramp && is_negative_const(delta_ramp->stride) && is_one(mutate(delta_ramp->base < 0))) {
                    expr = const_true(delta_ramp->lanes);
                } else if (delta_ramp && is_negative_const(delta_ramp->stride) && is_one(mutate(delta_ramp->base + delta_ramp->stride * (delta_ramp->lanes - 1) >= 0))) {
                    expr = const_false(delta_ramp->lanes);
                } else if (delta_ramp && mod_rem.modulus > 0 && const_int(delta_ramp->stride, &ia) && 0 <= ia * (delta_ramp->lanes - 1) + mod_rem.remainder && ia * (delta_ramp->lanes - 1) + mod_rem.remainder < mod_rem.modulus) {
        ...
        void visit(const And* op, const Expr& self)
        {
            ...
            } else if (eq_a && neq_b && ((equal(eq_a->a, neq_b->a) && equal(eq_a->b, neq_b->b)) || (equal(eq_a->a, neq_b->b) && equal(eq_a->b, neq_b->a)))) {
                // a == b && a != b
                expr = const_false(op->type.lanes());
            } else if (eq_b && neq_a && ((equal(eq_b->a, neq_a->a) && equal(eq_b->b, neq_a->b)) || (equal(eq_b->a, neq_a->b) && equal(eq_b->b, neq_a->a)))) {
                // a != b && a == b
                expr = const_false(op->type.lanes());
            } else if ((not_a && equal(not_a->a, b)) || (not_b && equal(not_b->a, a))) {
                // a && !a
                expr = const_false(op->type.lanes());
            } else if (le_a && lt_b && equal(le_a->a, lt_b->b) && equal(le_a->b, lt_b->a)) {
                // a <= b && b < a
                expr = const_false(op->type.lanes());
            } else if (lt_a && le_b && equal(lt_a->a, le_b->b) && equal(lt_a->b, le_b->a)) {
                // a < b && b <= a
                expr = const_false(op->type.lanes());
            } else if (lt_a && lt_b && equal(lt_a->a, lt_b->b) && const_int(lt_a->b, &ia) && const_int(lt_b->a, &ib) && ib + 1 >= ia) {
                // (a < ia && ib < a) where there is no integer a s.t. ib < a < ia
                expr = const_false(op->type.lanes());
            } else if (lt_a && lt_b && equal(lt_a->b, lt_b->a) && const_int(lt_b->b, &ia) && const_int(lt_a->a, &ib) && ib + 1 >= ia) {
                // (ib < a && a < ia) where there is no integer a s.t. ib < a < ia
                expr = const_false(op->type.lanes());

            } else if (le_a && lt_b && equal(le_a->a, lt_b->b) && const_int(le_a->b, &ia) && const_int(lt_b->a, &ib) && ib >= ia) {
                // (a <= ia && ib < a) where there is no integer a s.t. ib < a <= ia
                expr = const_false(op->type.lanes());
            } else if (le_a && lt_b && equal(le_a->b, lt_b->a) && const_int(lt_b->b, &ia) && const_int(le_a->a, &ib) && ib >= ia) {
                // (ib <= a && a < ia) where there is no integer a s.t. ib < a <= ia
                expr = const_false(op->type.lanes());

            } else if (lt_a && le_b && equal(lt_a->a, le_b->b) && const_int(lt_a->b, &ia) && const_int(le_b->a, &ib) && ib >= ia) {
                // (a < ia && ib <= a) where there is no integer a s.t. ib <= a < ia
                expr = const_false(op->type.lanes());
            } else if (lt_a && le_b && equal(lt_a->b, le_b->a) && const_int(le_b->b, &ia) && const_int(lt_a->a, &ib) && ib >= ia) {
                // (ib < a && a <= ia) where there is no integer a s.t. ib <= a < ia
                expr = const_false(op->type.lanes());

            } else if (le_a && le_b && equal(le_a->a, le_b->b) && const_int(le_a->b, &ia) && const_int(le_b->a, &ib) && ib > ia) {
                // (a <= ia && ib <= a) where there is no integer a s.t. ib <= a <= ia
                expr = const_false(op->type.lanes());
            } else if (le_a && le_b && equal(le_a->b, le_b->a) && const_int(le_b->b, &ia) && const_int(le_a->a, &ib) && ib > ia) {
                // (ib <= a && a <= ia) where there is no integer a s.t. ib <= a <= ia
                expr = const_false(op->type.lanes());

            } else if (eq_a && neq_b && equal(eq_a->a, neq_b->a) && is_simple_const(eq_a->b) && is_simple_const(neq_b->b)) {
                // (a == k1) && (a != k2) -> (a == k1) && (k1 != k2)
                // (second term always folds away)
                expr = mutate(And::make(a, NE::make(eq_a->b, neq_b->b)));
            } else if (neq_a && eq_b && equal(neq_a->a, eq_b->a) && is_simple_const(neq_a->b) && is_simple_const(eq_b->b)) {
                // (a != k1) && (a == k2) -> (a == k2) && (k1 != k2)
                // (second term always folds away)
                expr = mutate(And::make(b, NE::make(neq_a->b, eq_b->b)));
            } else if (eq_a && eq_a->a.as<Variable>() && is_simple_const(eq_a->b) && expr_uses_var(b, eq_a->a.as<Variable>())) {
                // (somevar == k) && b -> (somevar == k) && substitute(somevar, k, b)
                expr = mutate(And::make(a, substitute(eq_a->a.as<Variable>(), eq_a->b, b)));
            } else if (eq_b && eq_b->a.as<Variable>() && is_simple_const(eq_b->b) && expr_uses_var(a, eq_b->a.as<Variable>())) {
                // a && (somevar == k) -> substitute(somevar, k1, a) && (somevar == k)
                expr = mutate(And::make(substitute(eq_b->a.as<Variable>(), eq_b->b, a), b));
            } else if (broadcast_a && broadcast_b && broadcast_a->lanes == broadcast_b->lanes) {
                // x8(a) && x8(b) -> x8(a && b)
        ...
        void visit(const Or* op, const Expr& self)
        {
            ...
            if (is_one(a)) {
                ...
                // a == b || a != b
                expr = const_true(op->type.lanes());
            } else if (neq_a && eq_b && ((equal(eq_b->a, neq_a->a) && equal(eq_b->b, neq_a->b)) || (equal(eq_b->a, neq_a->b) && equal(eq_b->b, neq_a->a)))) {
                // a != b || a == b
                expr = const_true(op->type.lanes());
            } else if ((not_a && equal(not_a->a, b)) || (not_b && equal(not_b->a, a))) {
                // a || !a
                ...
                // a < b || b <= a
                expr = const_true(op->type.lanes());
            } else if (lt_a && lt_b && equal(lt_a->a, lt_b->b) && const_int(lt_a->b, &ia) && const_int(lt_b->a, &ib) && ib < ia) {
                // (a < ia || ib < a) where ib < ia
                expr = const_true(op->type.lanes());
            } else if (lt_a && lt_b && equal(lt_a->b, lt_b->a) && const_int(lt_b->b, &ia) && const_int(lt_a->a, &ib) && ib < ia) {
                ...
                // (ib < a || a <= ia) where ib <= ia
                expr = const_true(op->type.lanes());
            } else if (le_a && le_b && equal(le_a->a, le_b->b) && const_int(le_a->b, &ia) && const_int(le_b->a, &ib) && ib <= ia + 1) {
                // (a <= ia || ib <= a) where ib <= ia + 1
                expr = const_true(op->type.lanes());
            } else if (le_a && le_b && equal(le_a->b, le_b->a) && const_int(le_b->b, &ia) && const_int(le_a->a, &ib) && ib <= ia + 1) {
            ...
            } else if (is_var_simple_const_comparison(b, &xvar_c) && and_a && ((is_var_simple_const_comparison(and_a->a, &xvar_a) && xvar_a == xvar_c) || (is_var_simple_const_comparison(and_a->b, &xvar_b) && xvar_b == xvar_c))) {
                // (a && b) || (c) -> (a || c) && (b || c)
            ...
            } else if (is_var_simple_const_comparison(a, &xvar_c) && and_b && ((is_var_simple_const_comparison(and_b->a, &xvar_a) && xvar_a == xvar_c) || (is_var_simple_const_comparison(and_b->b, &xvar_b) && xvar_b == xvar_c))) {
                // (c) || (a && b) -> (a || c) && (b || c)
                // iff c and at least one of a or b is of the form
                //     (var == const) or (var != const)
                // (and the vars are the same)
        ...
        void visit(const Select* op, const Expr& self)
        {
            ...
            if (is_zero(condition)) {
                ...
                expr = true_value;
            } else if (true_value.type().is_bool() && is_one(true_value) && is_zero(false_value)) {
                if (true_value.type().is_vector() && condition.type().is_scalar()) {
                    expr = Broadcast::make(condition, true_value.type().lanes());
                } else {
                    expr = condition;
                }
            } else if (true_value.type().is_bool() && is_zero(true_value) && is_one(false_value)) {
                if (true_value.type().is_vector() && condition.type().is_scalar()) {
        ...
        void visit(const Ramp* op, const Expr& self)
        {
            ...
            } else if (base.same_as(op->base) && stride.same_as(op->stride)) {
                expr = self;
            } else {
        ...
        void visit(const Call* op, const Expr& self)
        {
            ...
            */
            ...
            } else if (op->call_type == Call::PureExtern && op->name == "is_nan_f32") {
            ...
            } else if (op->call_type == Call::PureExtern && op->name == "sqrt_f32") {
                Expr arg = mutate(op->args[0]);
            ...
            } else if (op->call_type == Call::PureExtern && (op->name == "floor_f32" || op->name == "ceil_f32" || op->name == "round_f32" || op->name == "trunc_f32")) {
                ...
                } else if (call && call->call_type == Call::PureExtern && (call->name == "floor_f32" || call->name == "ceil_f32" || call->name == "round_f32" || call->name == "trunc_f32")) {
        ...
        {
            if (op->is_extract_element() && (op->vectors[0].as<Ramp>() || op->vectors[0].as<Broadcast>())) {
        ...
        Expr hoist_slice_vector(Expr e)
        {
            ...
            internal_assert(shuffle_a && shuffle_b && shuffle_a->is_slice() && shuffle_b->is_slice());
        ...
        Body simplify_let(const T* op, const Body& self)
        {
            ...
            while (1) {
                ...
                if (is_const(new_value)) {
                    ...
                    new_var = repl_var;
                } else if (shuffle && shuffle->is_concat() && ((var_a && !var_b) || (!var_a && var_b))) {
            ...
            if (new_op && new_op->var.same_as(op->var) && new_op->body.same_as(op->body) && new_op->value.same_as(op->value)) {
                return self;
            }
        ...
        void visit(const AssertStmt* op, const Stmt& self)
        {
            ...
            if (a && is_zero(a->condition)) {
                ...
                const bool const_false_conditions_expected = call && call->name == "halideir_error_specialize_fail";
        ...
        void visit(const For* op, const Stmt& self)
        {
            ...
            if (const_int(new_min, &new_min_int) && const_int(new_extent, &new_extent_int)) {
                bounds_tracked = true;
            ...
            if (is_no_op(new_body)) {
                stmt = new_body;
            } else if (op->min.same_as(new_min) && op->extent.same_as(new_extent) && op->body.same_as(new_body)) {
                stmt = self;
            } else {
        ...
        void visit(const Allocate* op, const Stmt& self)
        {
            ...
            if (body_if && op->condition.defined() && equal(op->condition, body_if->condition)) {
            ...
            } else if (all_extents_unmodified && body.same_as(op->body) && condition.same_as(op->condition) && new_expr.same_as(op->new_expr)) {
                stmt = self;
            } else {
                stmt = Allocate::make(op->buffer_var, op->type, new_extents,
                    condition, body,
        ...
        void visit(const Block* op, const Stmt& self)
        {
            ...
            if (is_no_op(first) && is_no_op(rest)) {
                stmt = Evaluate::make(0);
            } else if (is_no_op(first)) {
                stmt = rest;
            } else if (is_no_op(rest)) {
                stmt = first;
            } else if (let_first && let_rest && equal(let_first->value, let_rest->value) && expr_is_pure(let_first->value)) {
            ...
            } else if (if_first && if_rest && equal(if_first->condition, if_rest->condition) && expr_is_pure(if_first->condition)) {
            ...
            } else if (if_first && if_rest && !if_rest->else_case.defined() && expr_is_pure(if_first->condition) && expr_is_pure(if_rest->condition) && is_one(mutate((if_first->condition && if_rest->condition) == if_rest->condition))) {
