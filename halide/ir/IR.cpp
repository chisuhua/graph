...
EXPORT Expr::Expr(int8_t x)
    : Expr(Internal::IntImm::make(Int(8), x))
{
}
EXPORT Expr::Expr(int16_t x)
    : Expr(Internal::IntImm::make(Int(16), x))
{
}
EXPORT Expr::Expr(int32_t x)
    : Expr(Internal::IntImm::make(Int(32), x))
{
}
EXPORT Expr::Expr(int64_t x)
    : Expr(Internal::IntImm::make(Int(64), x))
{
}
EXPORT Expr::Expr(uint8_t x)
    : Expr(Internal::UIntImm::make(UInt(8), x))
{
}
EXPORT Expr::Expr(uint16_t x)
    : IRHandle(Internal::UIntImm::make(UInt(16), x))
{
}
EXPORT Expr::Expr(uint32_t x)
    : IRHandle(Internal::UIntImm::make(UInt(32), x))
{
}
EXPORT Expr::Expr(uint64_t x)
    : IRHandle(Internal::UIntImm::make(UInt(64), x))
{
}
EXPORT Expr::Expr(float16_t x)
    : IRHandle(Internal::FloatImm::make(Float(16), (double)x))
{
}
EXPORT Expr::Expr(float x)
    : IRHandle(Internal::FloatImm::make(Float(32), x))
{
}
EXPORT Expr::Expr(double x)
    : IRHandle(Internal::FloatImm::make(Float(64), x))
{
}
EXPORT Expr::Expr(const std::string& s)
...
namespace Internal {
...
#endif
    EXPORT Expr
    IntImm::make(Type t, int64_t value)
    {
    ...
    EXPORT Expr Select::make(Expr condition, Expr true_value, Expr false_value)
    {
        ...
        internal_assert(condition.type().is_scalar() || condition.type().lanes() == true_value.type().lanes())
    ...
    EXPORT Stmt AssertStmt::make(Expr condition, Expr message, Stmt body)
    {
        ...
        internal_assert(message.type() == Int(32) || message.as<StringImm>()) << "AssertStmt message must be an int or string:"
                                                                              << message << "\n";
    ...
    template <>
    EXPORT void ExprNode<IntImm>::accept(IRVisitor* v, const Expr& e) const { v->visit((const IntImm*)this, e); }
    template <>
    EXPORT void ExprNode<UIntImm>::accept(IRVisitor* v, const Expr& e) const { v->visit((const UIntImm*)this, e); }
    template <>
    EXPORT void ExprNode<FloatImm>::accept(IRVisitor* v, const Expr& e) const { v->visit((const FloatImm*)this, e); }
    template <>
    EXPORT void ExprNode<StringImm>::accept(IRVisitor* v, const Expr& e) const { v->visit((const StringImm*)this, e); }
    template <>
    EXPORT void ExprNode<Cast>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Cast*)this, e); }
    template <>
    EXPORT void ExprNode<Variable>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Variable*)this, e); }
    template <>
    EXPORT void ExprNode<Add>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Add*)this, e); }
    template <>
    EXPORT void ExprNode<Sub>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Sub*)this, e); }
    template <>
    EXPORT void ExprNode<Mul>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Mul*)this, e); }
    template <>
    EXPORT void ExprNode<Div>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Div*)this, e); }
    template <>
    EXPORT void ExprNode<Mod>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Mod*)this, e); }
    template <>
    EXPORT void ExprNode<Min>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Min*)this, e); }
    template <>
    EXPORT void ExprNode<Max>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Max*)this, e); }
    template <>
    EXPORT void ExprNode<EQ>::accept(IRVisitor* v, const Expr& e) const { v->visit((const EQ*)this, e); }
    template <>
    EXPORT void ExprNode<NE>::accept(IRVisitor* v, const Expr& e) const { v->visit((const NE*)this, e); }
    template <>
    EXPORT void ExprNode<LT>::accept(IRVisitor* v, const Expr& e) const { v->visit((const LT*)this, e); }
    template <>
    EXPORT void ExprNode<LE>::accept(IRVisitor* v, const Expr& e) const { v->visit((const LE*)this, e); }
    template <>
    EXPORT void ExprNode<GT>::accept(IRVisitor* v, const Expr& e) const { v->visit((const GT*)this, e); }
    template <>
    EXPORT void ExprNode<GE>::accept(IRVisitor* v, const Expr& e) const { v->visit((const GE*)this, e); }
    template <>
    EXPORT void ExprNode<And>::accept(IRVisitor* v, const Expr& e) const { v->visit((const And*)this, e); }
    template <>
    EXPORT void ExprNode<Or>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Or*)this, e); }
    template <>
    EXPORT void ExprNode<Not>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Not*)this, e); }
    template <>
    EXPORT void ExprNode<Select>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Select*)this, e); }
    template <>
    EXPORT void ExprNode<Load>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Load*)this, e); }
    template <>
    EXPORT void ExprNode<Ramp>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Ramp*)this, e); }
    template <>
    EXPORT void ExprNode<Broadcast>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Broadcast*)this, e); }
    template <>
    EXPORT void ExprNode<Call>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Call*)this, e); }
    template <>
    EXPORT void ExprNode<Let>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Let*)this, e); }
    template <>
    EXPORT void StmtNode<LetStmt>::accept(IRVisitor* v, const Stmt& s) const { v->visit((const LetStmt*)this, s); }
    template <>
    EXPORT void StmtNode<AttrStmt>::accept(IRVisitor* v, const Stmt& s) const { v->visit((const AttrStmt*)this, s); }
    template <>
    EXPORT void StmtNode<AssertStmt>::accept(IRVisitor* v, const Stmt& s) const { v->visit((const AssertStmt*)this, s); }
    template <>
    EXPORT void StmtNode<ProducerConsumer>::accept(IRVisitor* v, const Stmt& s) const { v->visit((const ProducerConsumer*)this, s); }
    template <>
    EXPORT void StmtNode<For>::accept(IRVisitor* v, const Stmt& s) const { v->visit((const For*)this, s); }
    template <>
    EXPORT void StmtNode<Store>::accept(IRVisitor* v, const Stmt& s) const { v->visit((const Store*)this, s); }
    template <>
    EXPORT void StmtNode<Provide>::accept(IRVisitor* v, const Stmt& s) const { v->visit((const Provide*)this, s); }
    template <>
    EXPORT void StmtNode<Allocate>::accept(IRVisitor* v, const Stmt& s) const { v->visit((const Allocate*)this, s); }
    template <>
    EXPORT void StmtNode<Free>::accept(IRVisitor* v, const Stmt& s) const { v->visit((const Free*)this, s); }
    template <>
    EXPORT void StmtNode<Realize>::accept(IRVisitor* v, const Stmt& s) const { v->visit((const Realize*)this, s); }
    template <>
    EXPORT void StmtNode<Prefetch>::accept(IRVisitor* v, const Stmt& s) const { v->visit((const Prefetch*)this, s); }
    template <>
    EXPORT void StmtNode<Block>::accept(IRVisitor* v, const Stmt& s) const { v->visit((const Block*)this, s); }
    template <>
    EXPORT void StmtNode<IfThenElse>::accept(IRVisitor* v, const Stmt& s) const { v->visit((const IfThenElse*)this, s); }
    template <>
    EXPORT void StmtNode<Evaluate>::accept(IRVisitor* v, const Stmt& s) const { v->visit((const Evaluate*)this, s); }
    template <>
    EXPORT void ExprNode<Shuffle>::accept(IRVisitor* v, const Expr& e) const { v->visit((const Shuffle*)this, e); }
