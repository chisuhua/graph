...
namespace Internal {
    ...
    {
        if (e.as<IntImm>() || e.as<UIntImm>() || e.as<FloatImm>() || e.as<StringImm>()) {
            return true;
    ...
    bool is_zero(const Expr& e)
    {
        ...
        if (const Call* c = e.as<Call>()) {
            return (c->is_intrinsic(Call::bool_to_mask) || c->is_intrinsic(Call::cast_mask)) && is_zero(c->args[0]);
        }
        return false;
    }
    ...
    bool is_one(const Expr& e)
    {
        ...
        if (const Call* c = e.as<Call>()) {
            return (c->is_intrinsic(Call::bool_to_mask) || c->is_intrinsic(Call::cast_mask)) && is_one(c->args[0]);
        }
        return false;
    }
...
Expr print(const std::vector<Expr>& args)
{
    ...
    // Call halideir_print.
    Expr print_call = Internal::Call::make(Int(32), "halideir_print",
        { combined_string }, Internal::Call::Extern);

    // Return the first argument.
    Expr result = Internal::Call::make(args[0].type(), Internal::Call::return_second,
...
Expr require(const Expr& condition, const std::vector<Expr>& args)
{
    ...
    Expr requirement_failed_error = Internal::Call::make(Int(32),
