...
bool Type::can_represent(Type other) const
{
    if (lanes() != other.lanes())
        return false;
    if (is_int()) {
        return ((other.is_int() && other.bits() <= bits()) || (other.is_uint() && other.bits() < bits()));
    } else if (is_uint()) {
        return other.is_uint() && other.bits() <= bits();
    } else if (is_float()) {
        return ((other.is_float() && other.bits() <= bits()) || (bits() == 64 && other.bits() <= 32) || (bits() == 32 && other.bits() <= 16));
    } else {
        return false;
    }
}
...
{
    const CPPHandleTypeInfo* first = m_handle_type;
    const CPPHandleTypeInfo* second = other.m_handle_type;
    ...
    if (first == nullptr) {
        first = CPPHandleTraits<void*>::type_info();
    }
    if (second == nullptr) {
        second = CPPHandleTraits<void*>::type_info();
    }

    return first->m_inner_name == second->m_inner_name && first->m_namespaces == second->m_namespaces && first->m_enclosing_types == second->m_enclosing_types && first->m_cpp_type_modifiers == second->m_cpp_type_modifiers && first->m_reference_type == second->m_reference_type;
}
