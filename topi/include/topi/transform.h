    ...
    std::string tag = kMatMul)
{
    ...
    auto func =
        [&A, &B, &iter_vars, axes](const Array<Var>& input_indices) {
    ...
    std::string tag = kMatMul)
{
    ...
    auto func =
        [&A, &B, &iter_vars, A_axes_val, B_axes_val](const Array<Var>& input_indices) {
            int idx_input = 0;
            Array<Expr> A_indices;
            for (unsigned i = 0; i < A->shape.size(); ++i) {
...
inline Tensor arange(const Expr& start,
    ...
    std::string tag = kInjective)
{
    Expr num_elem = tvm::cast(tvm::Int(32), tvm::ceil(tvm::cast(tvm::Float(32), stop - start) / step));
    Array<Expr> shape;
    return compute(
        { num_elem }, [&](const Array<Var>& indices) {
    ...
    const std::string& tag = kInjective)
{
    ...
    return compute(
        out_ndarray_size, [&](const Array<Var>& indices) {
            Expr ret = 1;
            for (int i = 0; i < ndim; ++i) {
                ret *= src->shape[i];
            }
    ...
    const std::string tag = kInjective)
{
    ...
    return compute(
        oshape, [&](const Array<Var>& iter_vars) {
