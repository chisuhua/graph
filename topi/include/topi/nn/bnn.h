...
*/
        ...
        std::string tag = "binarize_pack")
    {
        ...
        for (size_t i = 0; i < n; ++i) {
            oshape.push_back(i == static_cast<size_t>(axis) ? tvm::ir::Simplify(ishape[i] / 32) : ishape[i]);
        }
        ...
        return tvm::compute(
            ...
            [&](const Array<Var>& indices) {
                ...
                for (size_t i = 0; i < n; ++i) {
                    start_idx.push_back(i == static_cast<size_t>(axis) ? indices[i] * 32 : static_cast<Expr>(indices[i]));
                }
                auto packed = make_const(UInt(32), 0);
                for (size_t j = 0; j < 32; ++j) {
                    Array<Expr> idx;
                    for (size_t i = 0; i < n; ++i) {
                        idx.push_back(i == static_cast<size_t>(axis) ? start_idx[i] + static_cast<int>(j) : start_idx[i]);
                    }
