...
namespace relay {
    ...
    public:
        explicit Unifier(TypeSolver* solver, const NodeRef& loc)
            : solver_(solver)
            , loc(loc)
        {
        }
        ...
        Type VisitType_(const TensorTypeNode* op, const Type& tn) final
        {
            ...
            if (tt1->shape.size() != tt2->shape.size()) {
                this->solver_->ReportError(
                    RELAY_ERROR(
                        "tensor type `" << PrettyPrint(tt1) << "` has " << tt1->shape.size() << " dimensions, while `" << PrettyPrint(tt2) << "` has " << tt2->shape.size() << " dimensions"),
                    this->loc);
                return Type(nullptr);
            }
    ...
    public:
        explicit Resolver(TypeSolver* solver)
            : solver_(solver)
        {
        }
    ...
    public:
        explicit Merger(TypeSolver* solver)
            : solver_(solver)
        {
        }
