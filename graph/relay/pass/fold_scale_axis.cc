 ...
 */
            ...
            const Message& message)
        {
            ...
            if (MatchBroadcastToLeftAxes(tlhs, trhs, expected_out_axes, &rhs) && (!message->require_positive || IsAllPositiveConstant(rhs))) {
                rnode->value = lhs;
                rnode->scale = rhs;
                rnode->axes = expected_out_axes;
                return Expr(rnode);
            } else if (MatchBroadcastToLeftAxes(trhs, tlhs, expected_out_axes, &lhs) && (!message->require_positive || IsAllPositiveConstant(lhs))) {
                rnode->value = rhs;
                rnode->scale = lhs;
                rnode->axes = expected_out_axes;
                return Expr(rnode);
        ...
        Array<Message> Conv2DForwardPrep(const Call& call, const Message& out_message)
        {
            ...
            if (kernel_layout.IndexOf(LayoutAxis::Get('i')) < 0 && c_small_axis < 0 && (param->groups == 1 || is_depthwise_conv2d)) {
 ...
 */
        ...
        class BackwardTransformerNode : public Node,
                                        private ExprMutator {
        public:
   ...
   */
        ...
        Message AddSubBackwardPrep(const Call& call, const Array<Message>& in_messages)
        {
            ...
            if (in_messages[0].defined() && MatchBroadcastToLeftAxes(tlhs, trhs, in_messages[0]->axes)) {
                return in_messages[0];
            } else if (in_messages[1].defined() && MatchBroadcastToLeftAxes(trhs, tlhs, in_messages[1]->axes)) {
                return in_messages[1];
            } else if (in_messages[0].defined() && in_messages[1].defined() && equal(in_messages[0]->axes, in_messages[1]->axes) && equal(tlhs->shape, trhs->shape)) {
                // add of two elements.
                return in_messages[0];
            } else {
            ...
            const BackwardTransformer& transformer)
        {
            ...
            if (lhs_message.defined()) {
                ...
                if (MatchBroadcastToLeftAxes(tlhs, trhs, lhs_message->axes, &rhs) && (!lhs_message->require_positive || IsAllPositiveConstant(rhs))) {
                ...
                if (MatchBroadcastToLeftAxes(trhs, tlhs, rhs_message->axes, &lhs) && (!rhs_message->require_positive || IsAllPositiveConstant(lhs))) {
        ...
        Message Conv2DBackwardPrep(const Call& call, const Array<Message>& in_messages)
        {
            ...
            // TODO(tvm-team) handle grouped conv by reshape + bcast
            bool is_depthwise_conv2d = IsDepthwiseConv2D(call, param, kernel_layout);
            if (kernel_layout.IndexOf(LayoutAxis::Get('o')) < 0 && kernel_layout.IndexOf(LayoutAxis::Get('i')) < 0 && c_small_axis < 0 && (param->groups == 1 || is_depthwise_conv2d)) {
