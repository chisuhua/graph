    ...
    private:
        ...
        bool IsCompatibleConv2D(const CallNode* a, const CallNode* b)
        {
            ...
            const auto shape_a = BijectiveLayoutNode::make(
                Layout(attrs_a->kernel_layout), kOIHW)
                                     .ForwardShape(tweight_a->shape);
            const auto shape_b = BijectiveLayoutNode::make(
                Layout(attrs_b->kernel_layout), kOIHW)
                                     .ForwardShape(tweight_b->shape);

            return eq(attrs_a->strides, attrs_b->strides) && eq(attrs_a->padding, attrs_b->padding) && eq(attrs_a->dilation, attrs_b->dilation) && eq(attrs_a->groups, attrs_b->groups) && eq(attrs_a->data_layout, attrs_b->data_layout) && eq(attrs_a->kernel_layout, attrs_b->kernel_layout) && eq(attrs_a->out_dtype, attrs_b->out_dtype) && eq(attrs_a->out_layout, attrs_b->out_layout) && eq(shape_a[2], shape_b[2]) && eq(shape_a[3], shape_b[3]);
        }
    ...
    private:
        ...
        bool IsArgCompatible(const CallNode* a, const CallNode* b, size_t index, size_t channel_pos)
        {
            ...
            if ((arg_channel_pos > channel_pos) || // size_t overflow
                !eq(ta->shape[arg_channel_pos], toutput_a->shape[channel_pos]) || !eq(tb->shape[arg_channel_pos], toutput_b->shape[channel_pos]))
                return false;

            for (size_t i = 0; i < ta->shape.size(); i++) {
                if (i == arg_channel_pos)
                    continue;
        ...
        bool CheckLevel(const Group& branches, size_t depth, size_t channel_pos, size_t parent_index)
        {
            ...
            for (auto it = branches.begin() + 1; it != branches.end(); it++) {
                ...
                if (!branch[depth]->op.same_as(call->op) || !attrs_equal(branch[depth]->attrs, call->attrs) || branch[depth]->args.size() != call->args.size()) {
                    return false;
                }
                ...
                for (size_t i = 0; i < call->args.size(); i++) {
                    ...
                    if (!IsArgCompatible(call, branch[depth], i, channel_pos) || !attrs_equal(call->attrs, branch[depth]->attrs)) {
                        return false;
                    }
                }
            }
        ...
        void CombineBranches(const Group& branches)
        {
            ...
            const std::string& layout = conv_param->out_layout == "" ? conv_param->data_layout : conv_param->out_layout;
            ...
            for (i = 1; i < depth; i++) {
                ...
                for (parent_index = 0; parent_index < branches[0][i]->args.size(); parent_index++) {
                    if (branches[0][i]->args[parent_index].get() == branches[0][i - 1])
                        break;
                }
                CHECK_NE(parent_index, branches[0][i]->args.size());
                if (!CheckLevel(branches, i, channel_pos, parent_index))
                    break;
