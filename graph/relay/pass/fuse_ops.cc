 ...
 */
#include "../../../compiler/common/arena.h"
   ...
   */
    ...
    private:
        ...
        void VisitExpr_(const ConstantNode* op) final
        {
            ...
            bool is_simple_const = (dtype == Int(32) || dtype == Int(64) || dtype == Float(32) || dtype == Float(64) || dtype == Bool());
 ...
 */
    ...
    private:
            ...
            F fcond)
        {
            ...
            for (auto link = src->outputs.head; link != nullptr; link = link->next) {
                if (!CheckPath_(link->value.node, sink, fcond))
                    return false;
            }
            return true;
        }
   ...
   */
            ...
            int phase)
        {
            for (size_t nid = 0; nid < groups_.size(); ++nid) {
                ...
                if (phase == 2) {
                    ...
                    if (group_node->pattern > kInjective)
                        continue;
                    ...
                    if (dom_root_group->pattern == kTuple)
                        continue;
                ...
                if (groups_[dom_parent_gindex] != nullptr && group_node->FindRoot() == groups_[dom_parent_gindex]->FindRoot()) {
                    continue;
                }
                ...
                if (group_node->pattern == kOutEWiseFusable) {
                    ...
                    if (dom_node->parent != nullptr && (dom_node->pattern <= kInjective || dom_node->pattern == kCommReduce)) {
    ...
    struct GlobalVarLiveness : ExprVisitor {
        ...
        explicit GlobalVarLiveness(const Module& mod)
            : module(mod)
            , visited()
        {
        }
