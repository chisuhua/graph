   ...
   */
    ...
    public:
        ...
        Expr VisitExpr_(const TupleGetItemNode* op) final
        {
            ...
            if (NeedDeviceCopy(tuple.operator->(), op)) {
                Expr new_expr = TupleGetItemNode::make(GetDeviceCopyExpr(tuple, op), op->index);
        ...
        Expr VisitExpr_(const IfNode* if_node) final
        {
            ...
            if (if_node->cond.same_as(cond) && if_node->true_branch.same_as(true_br) && if_node->false_branch.same_as(false_br)) {
    ...
    private:
        ...
        Expr GetDeviceCopyExpr(const Expr& src, const ExprNode* dst)
        {
            ...
            if (!NeedDeviceCopy(src_node, dst))
                return src;
   ...
   */
        ...
        int BottomUpPropagation()
        {
            ...
            for (auto it = post_visitor_.post_dfs_order_.crbegin();
                ...
                if (const auto* node = GetDeviceCopyNode(it->first)) {
                    ...
                    if (out_dev_type == -1)
                        out_dev_type = attrs->dst_dev_type;
                    if (it->second)
                        device_map_.Set(GetRef<Expr>(it->first),
                            attrs->dst_dev_type);
        ...
        void FillPropagation(int out_dev_type)
        {
            for (const auto& it : post_visitor_.post_dfs_order_) {
                Expr expr = GetRef<Expr>(it.first);
                if (!it.second)
                    device_map_.Set(expr, out_dev_type);
            }
        }
