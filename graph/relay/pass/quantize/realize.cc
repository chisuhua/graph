    ...
    namespace quantize {
            ...
            if (const auto* n = new_args[0].as<QRealizeIntExprNode>()) {
                ...
                if (static_cast<int>(shift_nbit) == shift_nbit) {
                    if (shift_nbit > 0) {
                        ...
                        data = LeftShift(data, MakeConstantScalar(cfg->dtype_activation, static_cast<int>(shift_nbit)));
                    }
                    data = Clip(data, clip_min_imm, clip_max_imm);
                    return QRealizeIntExprNode::make(data, dom_scale, n->dtype);
                } else {
            ...
            DataType* dtype_ptr, Expr* scale_ptr)
        {
            ...
            for (size_t i = 0; i < ret.size(); ++i) {
                ...
                } else if (ref_arg && ref_arg->op.same_as(simulated_quantize) && ref_arg->attrs.as<SimulatedQuantizeAttrs>()->kind == kQInput) {
