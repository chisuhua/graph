...
namespace relay {
    ...
    public:
        TypeVarTVisitor(
            ...
            , bound_type_vars_(bound_type_vars)
        {
        }
    ...
    public:
        explicit TypeVarEVisitor(const Module& mod)
            : mod_(mod)
        {
        }
 ...
 */
    ...
    bool IsAllPositiveConstant(const Expr& expr)
    {
        ...
        if (const auto* constant = expr.as<ConstantNode>()) {
            ...
            // tail recursion.
            if (op->op.same_as(expand_dims) || op->op.same_as(reshape) || op->op.same_as(transpose) || op->op.same_as(squeeze)) {
    ...
    Expr TypeSubst(const Expr& expr, const tvm::Map<TypeVar, Type>& subst_map)
    {
        ...
        public:
            explicit TypeSubstMutator(const tvm::Map<TypeVar, Type>& subst_map)
