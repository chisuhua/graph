 ...
 */
    ...
    {
        if (tlhs->shape.size() < trhs->shape.size())
            return false;
        AttrsEqual equal;
 ...
 */
        ...
        const Layout& kernel_layout)
    {
        ...
        return is_const_int(wshape[0], param->groups) && is_const_int(wshape[1], 1);
    }
    ...
    inline Constant MakeConstantScalar(DataType dtype, T value)
    {
        ...
        TVM_DTYPE_DISPATCH(dtype, DType, {
            if (dtype == Float(16)) {
                // convert to float16
                // storage is uint16_t
                *static_cast<DType*>(arr->data) = __truncXfYf2__<float, uint32_t, 23, uint16_t, uint16_t, 10>(static_cast<float>(value));
            } else {
