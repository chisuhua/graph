 ...
 */
   ...
   */
        Doc GetMetaNode(const NodeRef& node)
        {
            ...
            Array<NodeRef>& mvector = meta_data_[type_key];
   ...
   */
    ...
    class PrettyPrinter : public ExprFunctor<Doc(const Expr&)>,
   ...
   */
        ...
        {
            return !(dg_.expr_node.at(expr)->parents.head && dg_.expr_node.at(expr)->parents.head->next);
        }

        bool AlwaysInline(const Expr& expr)
        {
            return expr.as<GlobalVarNode>() || expr.as<ConstantNode>() || expr.as<OpNode>() || expr.as<VarNode>();
        }
 ...
 */
    ...
    public:
        AttrPrinter(std::vector<Doc>* doc, PrettyPrinter* parent)
            : docs(doc)
            , parent_(parent)
        {
        }
