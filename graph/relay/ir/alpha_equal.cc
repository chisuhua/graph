 ...
 */
#include "../../../compiler/lang/attr_functor.h"
...
namespace relay {
    ...
    class AlphaEqualHandler : public AttrsEqualHandler,
   ...
   */
        bool Equal(const NodeRef& lhs, const NodeRef& rhs)
        {
            ...
            if (const auto lhsm = lhs.as<ModuleNode>()) {
                ...
                if (!rhsm)
                    return false;
                if (lhsm->functions.size() != rhsm->functions.size())
                    return false;
                for (const auto& p : lhsm->functions) {
                    if (!Equal(p.second, rhsm->Lookup(p.first->name_hint)))
                        return false;
                }
                if (lhsm->type_definitions.size() != rhsm->type_definitions.size())
                    return false;
                for (const auto& p : lhsm->type_definitions) {
                    if (!Equal(p.second, rhsm->LookupDef(p.first->var->name_hint)))
                        return false;
                }
                return true;
            }
        ...
        bool AttrEqual(const NodeRef& lhs, const NodeRef& rhs)
        {
            auto compute = [&]() {
                ...
                if (auto lhsd = lhs.as<DictAttrsNode>()) {
                    ...
                    if (!rhsd)
                        return false;
                    if (lhsd->dict.size() != rhsd->dict.size())
                        return false;
                    for (const auto& k : lhsd->dict) {
                        if (!Equal(k.second, rhsd->dict[k.first]))
                            return false;
                    }
                    return true;
                }
   ...
   */
        ...
        bool VisitType_(const TensorTypeNode* lhs, const Type& other) final
        {
            if (const TensorTypeNode* rhs = other.as<TensorTypeNode>()) {
                return (lhs->dtype == rhs->dtype && AttrEqual(lhs->shape, rhs->shape));
            } else {
                return false;
            }
        }
        ...
        bool VisitType_(const FuncTypeNode* lhs, const Type& other) final
        {
            if (const FuncTypeNode* rhs = other.as<FuncTypeNode>()) {
                if (lhs->arg_types.size() != rhs->arg_types.size())
                    return false;
                if (lhs->type_params.size() != rhs->type_params.size())
                    return false;
                if (lhs->type_constraints.size() != rhs->type_constraints.size())
                    return false;
                ...
                for (size_t i = 0; i < lhs->arg_types.size(); i++) {
                    if (!TypeEqual(lhs->arg_types[i], rhs->arg_types[i]))
                        return false;
                }
                if (!TypeEqual(lhs->ret_type, rhs->ret_type))
                    return false;
        ...
        bool VisitType_(const TypeRelationNode* lhs, const Type& other) final
        {
            if (const TypeRelationNode* rhs = other.as<TypeRelationNode>()) {
                if (lhs->func->name != rhs->func->name)
                    return false;
                if (lhs->num_inputs != rhs->num_inputs)
                    return false;
                if (!this->AttrEqual(lhs->attrs, rhs->attrs))
                    return false;
        ...
        bool VisitType_(const TupleTypeNode* lhs, const Type& other) final
        {
            if (const TupleTypeNode* rhs = other.as<TupleTypeNode>()) {
                ...
                for (size_t i = 0; i < lhs->fields.size(); ++i) {
                    if (!TypeEqual(lhs->fields[i], rhs->fields[i]))
                        return false;
                }
                return true;
            } else {
        ...
        bool VisitExpr_(const VarNode* lhs, const Expr& other) final
        {
            ...
            if (const VarNode* rhs = other.as<VarNode>()) {
                if (lhs->name_hint() != rhs->name_hint())
                    return false;
                if (!TypeEqual(lhs->type_annotation, rhs->type_annotation))
                    return false;
                return LeafNodeEqual(GetRef<NodeRef>(lhs), other);
            } else {
                return false;
        ...
        bool VisitExpr_(const TupleNode* lhs, const Expr& other) final
        {
            if (const TupleNode* rhs = other.as<TupleNode>()) {
                if (lhs->fields.size() != rhs->fields.size())
                    return false;
                for (size_t i = 0; i < lhs->fields.size(); ++i) {
                    if (!ExprEqual(lhs->fields[i], rhs->fields[i]))
                        return false;
                }
                return true;
            } else {
        ...
        bool VisitExpr_(const FunctionNode* lhs, const Expr& other) final
        {
            if (const FunctionNode* rhs = other.as<FunctionNode>()) {
                ...
                for (size_t i = 0; i < lhs->params.size(); ++i) {
                    if (!MergeVarDecl(lhs->params[i], rhs->params[i]))
                        return false;
                }
                // check return types.
                if (!TypeEqual(lhs->ret_type, rhs->ret_type))
                    return false;
                if (!AttrEqual(lhs->attrs, rhs->attrs))
                    return false;
            ...
            if (const CallNode* rhs = other.as<CallNode>()) {
                ...
                if (!is_primitive) {
                    for (size_t i = 0; i < lhs->type_args.size(); ++i) {
                        if (!TypeEqual(lhs->type_args[i], rhs->type_args[i]))
                            return false;
                    }
                }
        ...
        bool VisitExpr_(const LetNode* lhs, const Expr& other) final
        {
            if (const LetNode* rhs = other.as<LetNode>()) {
                if (!MergeVarDecl(lhs->var, rhs->var))
                    return false;
                if (!ExprEqual(lhs->value, rhs->value))
                    return false;
                return ExprEqual(lhs->body, rhs->body);
            } else {
                return false;
        ...
        bool VisitExpr_(const IfNode* lhs, const Expr& other) final
        {
            if (const IfNode* rhs = other.as<IfNode>()) {
                return ExprEqual(lhs->cond, rhs->cond) && ExprEqual(lhs->true_branch, rhs->true_branch) && ExprEqual(lhs->false_branch, rhs->false_branch);
            } else {
                return false;
            }
        }
