 ...
 */
    struct Frame {
        ...
        explicit Frame(tvm::Map<Var, Value> locals)
            : locals(locals)
        {
        }
    };
 ...
 */
   ...
   */
        struct LocalFrame {
            Stack& st;
            explicit LocalFrame(Stack& st, const Frame& fr)
                : st(st)
            {
    ...
    class Interpreter : public ExprFunctor<Value(const Expr& n)>,
    ...
    public:
        ...
        inline Value MakeClosure(const Function& func, Var letrec_name = Var())
        {
            ...
            auto mut_closure = static_cast<ClosureNode*>(const_cast<Node*>(closure.get()));
