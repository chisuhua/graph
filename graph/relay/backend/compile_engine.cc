...
namespace relay {
    ...
    class ScheduleGetter : public ExprFunctor<Array<Tensor>(const Expr&)> {
    public:
        ...
        {
            static auto fschedule = Op::GetAttr<FTVMSchedule>("FTVMSchedule");
            ...
            if (master_attrs_.as<DeviceCopyAttrs>() == nullptr) {
                schedule = fschedule[master_op_](master_attrs_, tensor_outs, target_);
    ...
    private:
        ...
        CCacheValue LowerInternal(const CCacheKey& key)
        {
            ...
            if (it != cache_.end()) {
                ...
                if (it->second->cached_func.defined())
                    return it->second;
                value = it->second;
            } else {
