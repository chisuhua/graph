        ...
        const TypeReporter& reporter)
    {
        ...
        if (param->kernel_size.defined() && param->channels.defined()) {
            ...
            if (param->kernel_size.defined()) {
                CHECK_EQ(param->kernel_size.size(), 2);
                // check the size
                CHECK(reporter->AssertEQ(param->kernel_size[0], wshape[2]) && reporter->AssertEQ(param->kernel_size[1], wshape[3]))
        ...
        oshape.Set(2, (param->strides[0] * (dshape_nchw[2] - 1) + dilated_ksize_y - 2 * param->padding[0] + param->output_padding[0]));
        oshape.Set(3, (param->strides[1] * (dshape_nchw[3] - 1) + dilated_ksize_x - 2 * param->padding[1] + param->output_padding[1]));
...
- **out**:  Output is 4D array of shape (batch_size, channels, out_height, out_width)
        ...
        const TypeReporter& reporter)
    {
        ...
        reporter->Assign(types[1], TensorTypeNode::make(Array<IndexExpr>(oshape), data->dtype));
        return true;
    }
